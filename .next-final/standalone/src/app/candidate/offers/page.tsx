                ...
                <div key={o.id} className="p-6 rounded-2xl bg-emerald-50/50 border border-emerald-200 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{o.dutyStation} • {o.salary} • Respond by {formatDate(o.acceptanceDeadline)}</p>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusBadgeClass(o.status)}`}>{o.status}</span>
                  </div>
                  <div className="pt-2 flex justify-end">
                    <Link href={`/candidate/offers/${o.id}`} className="flex items-center gap-1.5 rounded-xl bg-emerald-600 px-4 py-2 text-xs font-bold text-white shadow hover:bg-emerald-700">
