                ...
                <div key={iv.id} className="p-6 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500">{iv.application.vacancy.title} • {iv.format}</p>
                    </div>
                    <span className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusBadgeClass(iv.status)}`}>{iv.status.replace(/_/g, ' ')}</span>
                  </div>
                  <div className="grid grid-cols-2 gap-4 text-xs pt-2">
                    ...
                    <div>
                      ...
                      <span className="font-mono text-blue-600 font-bold break-all">{iv.meetingLink || iv.venue || '—'}</span>
                    </div>
                  </div>
                  <a href={`/api/calendar/interviews/${iv.id}`} className="inline-flex items-center gap-1.5 rounded-lg border border-blue-300 px-3 py-2 text-xs font-bold text-blue-700">
                    <CalendarPlus className="h-3.5 w-3.5" /> Add to calendar
                  </a>
                  {!['ATTENDED','DID_NOT_ATTEND','CANCELLED'].includes(iv.status) && <InterviewResponse id={iv.id} current={iv.candidateResponse} />}
                </div>
              ))
            )}
          </div>
