...
function ApplyForm() {
    ...
    fetch(`/api/public/vacancies?id=${encodeURIComponent(vacancyId)}`)
      ...
      .then((data) => {
        ...
        return Promise.all([
          ...
          fetch(`/api/candidate/applications?vacancyId=${encodeURIComponent(vacancyId)}`).then((response) => response.ok ? response.json() : null),
      ...
      .then(([found, data, profileData]) => {
        ...
        if (data?.application?.isDraft) {
          ...
          for (const answer of data.application.answers || []) {
            try { restored[answer.vacancyQuestionId] = JSON.parse(answer.answerJson) } catch {}
          }
          setAnswers(restored)
          const restoredDocuments: Record<string, string> = {}
          for (const requirement of found?.requiredDocuments || []) {
            const linked = data.application.files?.find((file: any) =>
              file.vacancyRequiredDocumentId === requirement.id
              || (!file.vacancyRequiredDocumentId && profileData?.profile?.documents?.some((doc: any) => doc.fileAssetId === file.fileAssetId && doc.documentType === requirement.documentType))
            )
            const candidateDocument = profileData?.profile?.documents?.find((doc: any) => doc.fileAssetId === linked?.fileAssetId && doc.documentType === requirement.documentType)
      ...
      .catch(() => {
        if (hasCachedVacancy) setError('Connection unavailable. Your locally saved application draft is shown below and will sync when connectivity returns.')
  ...
  const save = async (mode: 'DRAFT' | 'SUBMIT') => {
    ...
    try {
      const res = await fetch('/api/candidate/applications', {
        method: 'POST', headers: { 'Content-Type': 'application/json', ...(mode === 'SUBMIT' ? { 'Idempotency-Key': submissionKey.current } : {}) },
        body: JSON.stringify({ vacancyId, answers: formattedAnswers, documents: Object.entries(selectedDocuments).filter(([, candidateDocumentId]) => candidateDocumentId).map(([requirementId, candidateDocumentId]) => ({ requirementId, candidateDocumentId })), mode, declarationsAccepted }),
      })
      const data = await res.json()
      if (!res.ok) { setError(data.error || `Failed to ${mode === 'DRAFT' ? 'save' : 'submit'} application`); return false }
      if (mode === 'DRAFT') {
        setSaveStatus(`Draft saved to your account at ${new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`)
        setError('')
      }
      return data.applicationId as string
    } catch { setError('An error occurred while saving your application.'); return false }
  ...
  const returnToEdit = useCallback(() => {
    ...
    window.setTimeout(() => {
      window.history.replaceState(null, '', `${window.location.pathname}${window.location.search}#application-edit-form`)
  ...
  const questionVisible = (question: any) => {
    ...
    try {
      ...
      if (condition.operator === 'CONTAINS') return Array.isArray(actual) ? actual.includes(condition.value) : String(actual ?? '').includes(condition.value)
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        const response = await fetch('/api/candidate/applications', {
          ...
          body: JSON.stringify({ vacancyId, answers: Object.entries(answers).map(([vacancyQuestionId, answer]) => ({ vacancyQuestionId, answer })), documents: Object.entries(selectedDocuments).filter(([,candidateDocumentId])=>candidateDocumentId).map(([requirementId,candidateDocumentId])=>({requirementId,candidateDocumentId})), mode: 'DRAFT', declarationsAccepted: false }),
        })
        setSaveStatus(response.ok
  ...
  const selectedRequiredDocuments = requiredDocuments.filter((requirement: any) => selectedDocuments[requirement.id]).length
        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <div className="section-panel p-5 sm:p-8 space-y-6">
            <div className="border-b border-slate-100 pb-4">
              ...
              <p className="text-xs text-slate-500">{vacancy?.department?.name} • {vacancy?.dutyStation?.name}</p>
              <div className="mt-3"><SaveIndicator status={saveStatus} /></div>
            </div>

            <section aria-labelledby="application-checklist" className="grid gap-3 border-y border-stone-200 bg-stone-50 p-4 sm:grid-cols-3">
              <div>
                <h2 id="application-checklist" className="text-xs font-bold uppercase tracking-[0.1em] text-stone-500">Application checklist</h2>
                <p className="mt-1 text-sm font-semibold text-stone-900">{requiredQuestions.length + requiredDocuments.length + 1} required items</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-stone-500">Questions</p>
                <p className="mt-1 text-sm font-bold text-stone-900">{completedRequiredQuestions} of {requiredQuestions.length} complete</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-stone-500">Documents and declaration</p>
                <p className="mt-1 text-sm font-bold text-stone-900">{selectedRequiredDocuments + (declarationsAccepted ? 1 : 0)} of {requiredDocuments.length + 1} complete</p>
              </div>
            </section>

            {error && (
              <div role={error.startsWith('Draft saved') ? 'status' : 'alert'} className={`flex items-center gap-2 rounded-xl border p-4 text-xs font-medium ${error.startsWith('Draft saved')?'border-emerald-200 bg-emerald-50 text-emerald-800':'border-rose-200 bg-rose-50 text-rose-800'}`}>
            ...
            <form id="application-edit-form" ref={formRef} data-testid="application-edit-form" onSubmit={handleSubmit} className="scroll-mt-28 space-y-6">
              ...
              <div className="border-l-4 border-brand-600 bg-brand-50 p-4 text-sm text-stone-700 space-y-1">
                ...
                <p>
                  We will take a copy of your profile when you submit. Changes made later will not alter this application.
                </p>
              </div>
              ...
              {vacancy?.questions && vacancy.questions.length > 0 && (
                <div className="space-y-4 pt-2">
                  ...
                  {vacancy.questions.map((q: any) => {
                    ...
                    try { configuration = q.configurationJson ? JSON.parse(q.configurationJson) : {} } catch {}
                    const options: string[] = configuration.options || []
                    return <div key={q.id} className="space-y-1.5">
                      ...
                      /> : q.fieldType === 'YESNO' ? <select id={questionId} required={q.required} value={answers[q.id] ?? ''} onChange={(event) => setAnswers({ ...answers, [q.id]: event.target.value })} className="field-control"><option value="">Choose an answer</option><option value="YES">Yes</option><option value="NO">No</option></select>
                      : q.fieldType === 'SELECT' ? <select id={questionId} required={q.required} value={answers[q.id] ?? ''} onChange={(event) => setAnswers({ ...answers, [q.id]: event.target.value })} className="field-control"><option value="">Choose an answer</option>{options.map((option) => <option key={option}>{option}</option>)}</select>
                      : q.fieldType === 'MULTISELECT' ? <div className="space-y-1">{options.map((option) => { const selected = Array.isArray(answers[q.id]) ? answers[q.id] : []; return <label key={option} className="flex gap-2 text-xs"><input type="checkbox" checked={selected.includes(option)} onChange={(event) => setAnswers({ ...answers, [q.id]: event.target.checked ? [...selected, option] : selected.filter((item: string) => item !== option) })}/>{option}</label> })}</div>
                      : q.fieldType === 'DECLARATION' ? <label className="flex gap-2 text-xs"><input required={q.required} type="checkbox" checked={answers[q.id] === true} onChange={(event) => setAnswers({ ...answers, [q.id]: event.target.checked })}/>I confirm this declaration</label>
                      : q.fieldType === 'FILE' ? <div><input id={questionId} required={q.required && !answers[q.id]} type="file" accept=".pdf,.doc,.docx,.jpg,.jpeg,.png" onChange={(event) => void uploadQuestionFile(q.id, event.target.files?.[0])}/>{answers[q.id] && <p className="text-xs text-emerald-700">File attached.</p>}</div>
                      : <input id={questionId} required={q.required} type={q.fieldType === 'NUMBER' ? 'number' : q.fieldType === 'DATE' ? 'date' : 'text'} value={answers[q.id] ?? ''} onChange={(event) => setAnswers({ ...answers, [q.id]: event.target.value })} className="field-control" />}
                    </div>
                  })}
                </div>
              )}
              ...
              {vacancy?.requiredDocuments?.length > 0 && (
                <div className="space-y-4">
                  <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700 border-b border-slate-100 pb-2">Required documents</h3>
                  {vacancy.requiredDocuments.map((requirement: any) => {
                    const options = candidateDocuments.filter((document) => document.documentType === requirement.documentType)
                    return <div key={requirement.id} className="space-y-1">
                      <label htmlFor={`document-${requirement.id}`} className="block text-xs font-bold text-slate-900">{requirement.documentType} {requirement.required && <span className="text-rose-600">*</span>}</label>
                      <select id={`document-${requirement.id}`} required={requirement.required} value={selectedDocuments[requirement.id] || ''} onChange={(event) => setSelectedDocuments({ ...selectedDocuments, [requirement.id]: event.target.value })} className="field-control">
                        <option value="">Select a saved document</option>
                        {options.map((document: any) => <option key={document.id} value={document.id}>{document.fileAsset?.originalName || document.documentType}</option>)}
                      </select>
                      {!options.length && <p className="text-xs text-amber-700">Upload this document in <Link className="underline font-bold" href="/candidate/profile/documents">your document library</Link> before submitting.</p>}
                    </div>
                  })}
                </div>
              )}
              ...
              <div className="rounded-2xl bg-slate-50 border border-slate-200 p-5 space-y-3">
                ...
                <div className="space-y-2 text-xs text-slate-600">
                  ...
                  <p>I understand that false or misleading information may lead to disqualification or, if discovered later, dismissal.</p>
                </div>
              ...
              <div className="flex flex-wrap justify-end gap-3 pt-4">
                <button type="button" onClick={() => save('DRAFT')} disabled={submitting} className="btn-secondary">Save draft</button>
                <button
                  type="button"
                  onClick={() => {
          ...
          {visibleQuestions.map((question: any) => (
            <div key={question.id} className="border-t border-stone-200 pt-3">
              ...
              <p className="mt-1 whitespace-pre-wrap text-sm text-stone-900">
                {Array.isArray(answers[question.id]) ? answers[question.id].join(', ') : answers[question.id] === true ? 'Confirmed' : String(answers[question.id] ?? 'Not answered')}
              </p>
            </div>
          ))}
          <p className="border-t border-stone-200 pt-3 text-xs leading-5 text-stone-600">
            Submitting creates a fixed copy of your profile, answers and selected documents. You can withdraw later, but you cannot edit this application after submission.
          </p>
          <div className="flex justify-end gap-2">
            <button type="button" onClick={returnToEdit} className="btn-secondary">Go back and edit</button>
            <button type="button" disabled={submitting} onClick={(event) => { setReviewOpen(false); void handleSubmit(event as unknown as React.FormEvent) }} className="btn-primary">
