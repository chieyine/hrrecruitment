...
function assignedPolicyMethod(policy: { policySnapshotJson?: string | null; policyDocument?: { acknowledgementMethod?: string } | null }) {
  try { return JSON.parse(policy.policySnapshotJson || '{}').acknowledgementMethod || policy.policyDocument?.acknowledgementMethod || 'TYPED_NAME' }
  catch { return policy.policyDocument?.acknowledgementMethod || 'TYPED_NAME' }
}
            ...
            {policies.length === 0 ? (
              ...
              policies.map((p) => (
                <div key={p.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs">
                  <div className="flex items-center justify-between gap-4"><span className="font-bold text-slate-900">{p.policyDocument?.title || 'Policy'}</span><span className={`px-3 py-1 rounded-full font-bold border ${getStatusBadgeClass(p.status)}`}>{p.status.replace(/_/g, ' ')}</span></div>
                  {p.policyDocument?.summary && <p className="mt-2 text-slate-600">{p.policyDocument.summary}</p>}
                  {p.policyDocument?.fileAssetId && <a href={`/api/assets/download/${p.policyDocument.fileAssetId}`} className="mt-2 inline-block font-bold text-blue-700 hover:underline">Download policy</a>}
                  {!['SIGNED', 'APPROVED', 'WAIVED'].includes(p.status) && <PolicyAction resourceId={p.id} method={assignedPolicyMethod(p)} />}
                </div>
              ))
            )}
          </div>
