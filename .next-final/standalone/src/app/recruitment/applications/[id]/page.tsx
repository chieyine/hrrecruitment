...
export default function ApplicationReviewPage(props: { params: Promise<{ id: string }> }) {
  ...
  const outstanding = [
    ...
    ...(app.offers || []).filter((offer: any) => ['SENT', 'VIEWED'].includes(offer.status) && new Date(offer.acceptanceDeadline) < new Date(now + 48 * 60 * 60_000)).map((offer: any) => `Offer response is due ${formatDateTime(offer.acceptanceDeadline)}.`),
    ...(app.preboardings || []).filter((item: any) => item.readinessStatus !== 'READY_TO_RESUME' && item.confirmedStartDate && new Date(item.confirmedStartDate) < new Date(now + 7 * 86400000)).map(() => 'Preboarding is not ready for the confirmed start date.'),
