  ...
  return <div className="flex min-h-screen flex-col bg-slate-50">
    ...
    <main id="main-content" className="flex-1 py-10">
      <div className="mx-auto max-w-6xl space-y-6 px-4">
        ...
        <section className="section-panel p-5"><h2 className="font-bold">Panel scoring variance</h2>{variances.map((item) => <p key={item.id} className={`mt-2 border-t pt-2 text-sm ${item.spread >= 20 ? 'font-bold text-amber-800' : ''}`}><Link href={`/recruitment/interviews#interview-${item.id}`} className="hover:underline">{item.title}: {item.min.toFixed(1)}–{item.max.toFixed(1)} ({item.spread.toFixed(1)} spread, {item.count} panelists)</Link></p>)}{!variances.length && <p className="mt-2 text-sm text-slate-500">No multi-panel submissions yet.</p>}</section>
