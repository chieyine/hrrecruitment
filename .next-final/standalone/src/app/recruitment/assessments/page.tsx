    ...
    <div className="flex min-h-screen flex-col bg-slate-50">
      ...
      <main id="main-content" className="flex-1 py-10">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          <AssessmentManager vacancies={vacancies} eligible={eligibleApplications.map((a) => ({ id: a.id, name: `${a.candidate.legalFirstName} ${a.candidate.lastName}`, vacancyId: a.vacancyId }))} assessments={assessments.map((a) => ({ id: a.id, title: a.title, vacancyId: a.vacancyId, type: a.type, description: a.description, durationMinutes: a.durationMinutes, passMark: a.passMark, maximumAttempts: a.maximumAttempts, opensAt: a.opensAt, closesAt: a.closesAt, randomizeQuestions: a.randomizeQuestions, autoSubmit: a.autoSubmit, candidateCount: a._count.candidateAssessments }))} candidateAssessments={assessments.flatMap((assessment)=>assessment.candidateAssessments.map((record)=>({id:record.id,assessmentId:assessment.id,candidateName:`${record.application.candidate.legalFirstName} ${record.application.candidate.lastName}`,status:record.status})))} />
