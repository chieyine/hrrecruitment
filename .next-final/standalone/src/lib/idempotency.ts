...
export async function claimIdempotency(input: { request: Request; scope: string; actorUserId?: string | null; payload: unknown; ttlHours?: number }): Promise<IdempotencyClaim | null> {
  ...
  if (!Number.isFinite(ttlHours) || ttlHours <= 0) throw new AuthzError('Idempotency TTL must be greater than zero', 500)

  const replayOf = (existing: { requestHash: string; responseJson: string | null; statusCode: number | null }): IdempotencyClaim => {
    if (existing.requestHash !== requestHash) throw new AuthzError('Idempotency-Key was already used with a different request', 409)
    if (existing.responseJson !== null && existing.statusCode !== null) return { replay: true, statusCode: existing.statusCode, body: JSON.parse(existing.responseJson) }
    throw new AuthzError('An identical request is already being processed', 409)
  }

  for (let attempt = 0; attempt < 3; attempt++) {
    const now = new Date()
    const existing = await prisma.idempotencyRecord.findUnique({ where: { scope_key_actorUserId: { scope: input.scope, key, actorUserId } } })
