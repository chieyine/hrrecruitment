...
export async function runRetentionPolicy(now = new Date()) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const outboxApplicationIds = [...new Set(outboxCandidates.map((item) => item.applicationId).filter((id): id is string => Boolean(id)))]
      ...
      const [deletedDrafts, deletedNotifications, deletedReferences, deletedOutbox, deletedIdempotency, deletedRateLimitBuckets] = await Promise.all([
        draftIds.length ? tx.application.deleteMany({ where: { id: { in: draftIds } } }) : { count: 0 },
        notificationIds.length ? tx.notification.deleteMany({ where: { id: { in: notificationIds } } }) : { count: 0 },
        ...
        idempotencyIds.length ? tx.idempotencyRecord.deleteMany({ where: { id: { in: idempotencyIds } } }) : { count: 0 },
        rateLimitKeys.length ? tx.rateLimitBucket.deleteMany({ where: { keyHash: { in: rateLimitKeys } } }) : { count: 0 },
      ])
      ...
      const evidence = {
        ...
        activeHolds: activeHolds.map((hold) => ({ id: hold.id, resourceType: hold.resourceType, resourceId: hold.resourceId })),
        deletionManifest: {
