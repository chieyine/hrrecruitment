...
export default function Header({ currentUser }: { currentUser?: UserSession | null }) {
  ...
  const isCandidate = Boolean(resolvedUser?.roles?.length && resolvedUser.roles.every((role) => role === 'CANDIDATE'))
  ...
  const home = resolvedUser
    ? isCandidate ? '/candidate/dashboard' : isCourseAdmin ? '/admin/courses' : isApprover ? '/recruitment/approvals' : isPanelOnly ? '/recruitment/interviews' : isAuditorOnly ? '/recruitment/audit' : '/recruitment/work'
    : '/careers'
  const navLinks = resolvedUser
    ? isCandidate
      ? [
