...
export default function ConfigurationBuilder({ mode }: { mode: 'scorecards' | 'packages' | 'courses' }) {
  ...
  return (
    <section className="mt-6 space-y-4 border border-brand-200 bg-brand-50/40 p-5">
      <div>
...
function ScorecardEditor({ item, act, busy }: { item: any; act: Action; busy: boolean }) {
  ...
  return (
    <div>
      <button onClick={() => setOpen(true)} className="rounded bg-brand-700 px-3 py-2 text-xs font-bold text-white">
        Add criterion
      </button>
      {item.criteria.map((criterion: any) => (
        <div key={criterion.id} className="mt-2 flex justify-between gap-3 border bg-white p-2 text-xs">
          <span>
            {criterion.name} — max {criterion.maximumScore}, weight {criterion.weight}
          </span>
          <button
            disabled={busy}
    ...
    <div>
      <div className="flex flex-wrap items-end gap-2">
        <label className="text-xs font-semibold">
          ...
          <select
            ...
            className="mt-1 block rounded border bg-white p-2 text-xs"
          >
            <option>FORM</option>
        ...
        <label className="text-xs font-semibold">
          ...
          <select
            ...
            className="mt-1 block min-w-56 rounded border bg-white p-2 text-xs"
          >
        ...
        <label className="text-xs font-semibold">
          ...
          <input
            ...
            className="mt-1 block w-28 rounded border bg-white p-2 text-xs"
          />
        </label>
        {type === 'COURSE' && (
          <label className="text-xs font-semibold">
        ...
        <button
          ...
          className="rounded bg-brand-700 px-3 py-2 text-xs font-bold text-white disabled:opacity-50"
        >
          Add required item
        </button>
      </div>
      {groups.flatMap(([itemType, entries, relation]) =>
        entries.map((entry: any) => (
          <div key={entry.id} className="mt-2 flex justify-between gap-3 border bg-white p-2 text-xs">
            <span>
              {itemType}: {entry[relation]?.title || entry[relation]?.name}
              {itemType === 'COURSE' ? ` — ${String(entry.timing).replaceAll('_', ' ').toLowerCase()}` : ''}
            </span>
            <button
              disabled={busy}
