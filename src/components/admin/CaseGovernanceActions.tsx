    ...
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <div className="rounded-xl border border-slate-200 p-4">
        <h4 className="flex items-center gap-2 text-sm font-bold text-slate-900">
          <NotebookPen className="h-4 w-4 text-brand-700" aria-hidden /> Add a case note
        </h4>
        <p className="mt-1 text-xs text-slate-600">
          Part of the accountable record. Notes cannot be edited once saved.
        </p>
        ...
        <div className="rounded-xl border border-slate-200 p-4">
          ...
          {reopenable.length === 0 ? (
            ...
            <>
                ...
                {reopenable.map((scorecard) => (
                  <li key={scorecard.id} className="flex items-center justify-between gap-3 p-2 text-xs">
                    <span className="font-semibold text-slate-800">
                      {scorecard.scorecardType || 'Screening scorecard'}
                      {scorecard.reviewerName ? ` — ${scorecard.reviewerName}` : ''}
                    </span>
                    <button
                      type="button"
                      onClick={() => void viewScorecard(scorecard.id)}
                      disabled={busy !== null}
                      className="inline-flex items-center gap-1 font-bold text-brand-700 disabled:opacity-50"
                    >
                      {busy === `view:${scorecard.id}` ? (
              ...
              {viewedScorecard && (
                <div className="mt-3 border border-brand-200 bg-brand-50 p-3 text-xs">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="font-bold text-brand-950">
                        {viewedScorecard.scorecardTemplate?.name || 'Submitted scorecard'}
                      </p>
                      <p className="mt-1 text-brand-800">
                        Total: {viewedScorecard.totalScore ?? 'Not calculated'} · {viewedScorecard.status} · version{' '}
                        {viewedScorecard.version ?? 1}
                      </p>
                    </div>
                    <button type="button" onClick={() => setViewedScorecard(null)} className="font-bold text-brand-800">
                      Close
                    </button>
                  </div>
                  <dl className="mt-3 space-y-2">
                    {(viewedScorecard.criterionScores || []).map((score: any) => {
                      const criterion = viewedScorecard.scorecardTemplate?.criteria?.find(
                        (item: any) => item.id === score.criterionId
                      )
                      return (
                        <div key={score.id} className="border-t border-brand-200 pt-2">
                          <dt className="font-bold text-slate-900">
                            {criterion?.name || 'Criterion'} — {score.score}/{criterion?.maximumScore ?? '—'}
                          </dt>
                          <dd className="mt-1 whitespace-pre-wrap text-slate-700">
                            {score.comment || 'No comment recorded.'}
                          </dd>
                        </div>
                      )
                    })}
                  ...
                  {(() => {
                    ...
                    return previous.length > 0 ? (
                      <p className="mt-3 border-t border-brand-200 pt-2 text-brand-900">
                        {previous.length} previous submitted version{previous.length === 1 ? '' : 's'} retained in the
                        audit record.
                      </p>
                    ) : null
                  })()}
      ...
      {message && (
        <p
          role="status"
          className="md:col-span-2 rounded-lg bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-800"
        >
          {message}
        </p>
      )}
