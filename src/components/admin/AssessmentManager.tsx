...
export default function AssessmentManager({
  ...
  const create = async (event: React.FormEvent) => {
    ...
    const response = await fetch('/api/recruitment/assessments', {
      method: 'POST',
  ...
  const recordResult = async () => {
    ...
    const isOfflineRecord =
      ...
      ['OFFLINE_WRITTEN', 'PRACTICAL', 'PRESENTATION', 'DRIVING_TEST', 'SIMULATION'].includes(
        selectedAssessmentRecord.type
      )
    let scoreSheetFileId: string | undefined
    if (scoreSheet) {
      const form = new FormData()
      form.append('file', scoreSheet)
      form.append('sensitivityClass', 'CONFIDENTIAL')
      const upload = await fetch('/api/assets/upload', { method: 'POST', body: form })
      const body = await upload.json()
      if (!upload.ok) {
        setMessage(body.error || 'Score sheet upload failed')
        return
      }
      scoreSheetFileId = body.fileAssetId
    }
    const response = await fetch(`/api/recruitment/candidate-assessments/${resultId}/mark`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        score: Number(resultScore),
        comment: resultComment,
        ...(isOfflineRecord ? { offlineRecord: { ...offlineRecord, scoreSheetFileId } } : {}),
      }),
    })
    const data = await response.json()
  ...
  const resetAttempt = async () => {
    ...
    const data = await response.json()
    setMessage(response.ok ? 'Assessment attempt reset and candidate may try again.' : data.error || 'Reset failed')
    if (response.ok) {
      setResetId('')
      setResetReason('')
      router.refresh()
  ...
  const updateEditQuestion = (index: number, changes: Partial<Question>) =>
    setEditQuestions((current) => current.map((question, i) => (i === index ? { ...question, ...changes } : question)))
  const inputDate = (value: string | Date | null | undefined) =>
  ...
  const editAssessment = async (id: string) => {
    ...
    setEditBusy(false)
    if (!response.ok) {
      setMessage(data.error || 'Assessment could not be loaded.')
      return
    }
    ...
    setEditForm({
      ...
      opensAt: inputDate(item.opensAt),
      closesAt: inputDate(item.closesAt),
  ...
  const saveAssessment = async (event: React.FormEvent) => {
    ...
    const response = await fetch(`/api/recruitment/assessments/${editingId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...editForm,
        opensAt: editForm.opensAt || null,
        closesAt: editForm.closesAt || null,
        ...(selected?.candidateCount ? {} : { questions }),
      }),
    })
    const data = await response.json()
    setEditBusy(false)
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="order-1 grid gap-4 lg:grid-cols-2">
        <div className="rounded-2xl border bg-white p-5 space-y-3">
          ...
          <select
            ...
            className="w-full rounded border p-2 text-sm"
          >
            <option value="">Eligible candidate</option>
            {eligible
              .filter((a) => a.vacancyId === selectedAssessment?.vacancyId)
              .map((a) => (
                <option key={a.id} value={a.id}>
                  {a.name}
                </option>
        ...
        <div className="rounded-2xl border bg-white p-5 space-y-3">
          ...
          <select
            ...
            className="w-full rounded border p-2 text-sm"
          >
            <option value="">Candidate assessment</option>
          ...
          <button
            ...
            disabled={!resultId || resultScore === ''}
            className="rounded bg-purple-700 px-4 py-2 text-xs font-bold text-white disabled:opacity-50"
          >
            Record outcome
          </button>
        </div>
      ...
      <div className="order-2 rounded-2xl border bg-white p-5 space-y-3">
        ...
        <textarea
          ...
          placeholder="Reason for allowing another attempt"
          className="w-full rounded border p-2 text-sm"
        />
        <button
          type="button"
          onClick={resetAttempt}
          disabled={!resetId || resetReason.trim().length < 10}
          className="rounded bg-amber-700 px-4 py-2 text-xs font-bold text-white disabled:opacity-50"
        >
          Reset attempt
        </button>
      </div>
