      ...
      <main id="main-content" className="flex-1 py-8">
        <div className="page-shell space-y-7">
          <Link
            ...
            className="inline-flex items-center gap-1.5 text-xs font-semibold text-stone-500 hover:text-brand-700"
          >
          ...
          <div className="space-y-3">
            {preboardings.length === 0 ? (
              <EmptyState
                title="No candidates in preboarding"
                description="Candidates appear here after an accepted offer starts the preboarding process."
              />
            ) : (
              preboardings.map((pb) => {
                const c = pb.application.candidate
                const now = new Date()
                const overdue = [...pb.forms, ...pb.documents, ...pb.courses, ...pb.tasks].filter(
                  (item) =>
                    item.required &&
                    item.dueAt &&
                    item.dueAt < now &&
                    !['APPROVED', 'WAIVED', 'SIGNED', 'COMPLETED'].includes(item.status)
                ).length
                const blockers = pb.readinessChecks.filter(
                  (check) => check.required && !['PASSED', 'WAIVED'].includes(check.status)
                ).length
                return (
                  <div
                    key={pb.id}
                    className="section-panel flex flex-col justify-between gap-4 p-5 md:flex-row md:items-center"
                  >
                    <div>
                      <h4 className="font-bold text-slate-900 text-sm">
                        {c.legalFirstName} {c.lastName} — {pb.application.vacancy.title}
                      </h4>
                      <p className="text-xs text-slate-500">
                        Completion {pb.overallCompletionPercentage}% · {blockers} readiness blocker
                        {blockers === 1 ? '' : 's'} ·{' '}
                        <span className={overdue ? 'font-bold text-red-700' : ''}>{overdue} overdue</span>
                      </p>
                    </div>
                    <span
                      className={`px-3 py-1 rounded-full text-xs font-bold border ${getStatusBadgeClass(pb.readinessStatus)}`}
                    >
                      {pb.readinessStatus.replace(/_/g, ' ')}
                    </span>
