        ...
        <div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
          <div className="border-l-4 border-[#d4875f] bg-brand-900 p-7 text-white">
            <div className="flex flex-col justify-between gap-4 md:flex-row md:items-end">
              <div>
                <div className="mb-2 flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.18em] text-brand-200">
                  <ListTodo className="h-4 w-4" /> Work queue
                </div>
                <h1 className="font-display text-4xl font-normal">My work</h1>
                <p className="mt-2 max-w-2xl text-sm text-brand-100">
                  Reviews, approvals and candidate actions assigned to you.
                </p>
              </div>
              <div className="flex gap-2 text-xs font-bold">
                <Link
                  href="/recruitment/work?scope=mine"
                  className={`px-4 py-2 ${mineOnly ? 'bg-white text-brand-950' : 'border border-white/30'}`}
                >
                  My queue
                </Link>
                {canReadAll && (
          ...
          <div className="grid gap-4 sm:grid-cols-4">
            {[
              ['OPEN', 'Open', ListTodo, 'text-brand-700'],
              ...
              <Link
                key={String(value)}
                href={`/recruitment/work?status=${value}&scope=${mineOnly ? 'mine' : 'team'}`}
                className={`rounded-2xl border bg-white p-5 shadow-sm ${status === value ? 'border-brand-500 ring-2 ring-brand-100' : 'border-slate-200'}`}
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase tracking-wide text-slate-500">{String(label)}</span>
          ...
          <section aria-labelledby="attention-heading" className="section-panel">
            <div className="section-heading">
              <div>
                <h2 id="attention-heading" className="text-lg font-bold text-slate-950">
                  Needs attention
                </h2>
                <p className="mt-1 text-sm text-slate-600">
                  Exceptions that may delay candidates or require escalation.
                </p>
              </div>
              {attention && (
                <Link
                  href={`/recruitment/work?scope=${mineOnly ? 'mine' : 'team'}&status=OPEN`}
            ...
            <div className="grid gap-px overflow-hidden border border-slate-200 bg-slate-200 sm:grid-cols-4">
              {[
                ...
                key === 'blocked' ? (
                  <Link
                    key={String(key)}
                    href={`/recruitment/work?scope=${mineOnly ? 'mine' : 'team'}&status=BLOCKED`}
                    className="bg-white p-4 hover:bg-slate-50"
                  >
                  ...
                  <Link
                    key={String(key)}
                    href={`/recruitment/work?scope=${mineOnly ? 'mine' : 'team'}&attention=${key}`}
                    className="bg-white p-4 hover:bg-slate-50"
                  >
                    <span className="text-xs font-semibold text-slate-600">{label}</span>
                    <span className="mt-1 block text-2xl font-bold text-slate-950">{value}</span>
          ...
          <section aria-labelledby="elsewhere-heading" className="section-panel">
            <div className="section-heading">
              <div>
                <h2 id="elsewhere-heading" className="text-lg font-bold text-slate-950">
                  Elsewhere in recruitment
                </h2>
            ...
            <div className="grid gap-px border border-slate-200 bg-slate-200 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                [
                  ...
                  'Assessments',
                  'Create assessments, invite candidates and mark submissions',
                ],
          ...
          <section aria-labelledby="automation-heading" className="section-panel">
            <div className="section-heading">
              <div>
                <h2 id="automation-heading" className="flex items-center gap-2 text-lg font-bold text-slate-950">
                  <RefreshCw className="h-5 w-5 text-brand-700" />
                  Scheduled activity
                </h2>
                <p className="mt-1 text-sm text-slate-600">
                  Reminders, expiry checks, escalations and report deliveries completed in the last seven days.
                </p>
              </div>
              {(user.roles.includes('HR_MANAGER') || user.roles.includes('SYSTEM_ADMIN')) && (
                <Link href="/admin/automations" className="text-sm font-semibold text-brand-700 underline">
                  Manage schedules
                </Link>
              )}
            </div>
            <div className="grid gap-px border border-slate-200 bg-slate-200 sm:grid-cols-3">
              <div className="bg-white p-4">
                <p className="text-xs font-semibold text-slate-600">Actions completed</p>
                <p className="mt-1 text-2xl font-bold text-slate-950">{automated}</p>
              </div>
              <div className="bg-white p-4">
                <p className="text-xs font-semibold text-slate-600">Successful runs</p>
                <p className="mt-1 text-2xl font-bold text-slate-950">{recentRuns.length}</p>
              </div>
              <div className="bg-white p-4">
                <p className="text-xs font-semibold text-slate-600">Last completed</p>
                <p className="mt-2 text-sm font-bold text-slate-950">
                  {recentRuns[0] ? formatDate(recentRuns[0].completedAt || recentRuns[0].startedAt) : 'No run recorded'}
                </p>
              </div>
            </div>
          </section>
          ...
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-200 px-6 py-5">
              <h2 className="font-bold text-slate-900">
                {attention ? `${attention.replaceAll('_', ' ')} work` : `${status.replaceAll('_', ' ')} work`}
              </h2>
              <p className="mt-1 text-xs text-slate-500">
                Due work appears first. A record closes only when its underlying action is completed.
              </p>
            </div>
            <div className="divide-y divide-slate-100">
              {items.length === 0 ? (
                <div className="p-10 text-center text-sm text-slate-500">There is no work in this queue.</div>
              ) : (
                items.map((item) => {
                    ...
                    <div key={item.id} className="grid gap-4 p-5 md:grid-cols-[1fr_auto] md:items-center">
                      <div>
                        <div className="flex flex-wrap items-center gap-2">
                          <span
                            className={`rounded-full px-2.5 py-1 text-[10px] font-bold ${isOverdue ? 'bg-red-100 text-red-800' : 'bg-brand-50 text-brand-700'}`}
                          >
                            {isOverdue ? 'OVERDUE' : item.priority}
                          </span>
                          <span className="text-[11px] font-semibold text-slate-500">
                            {item.workType.replaceAll('_', ' ')}
                          </span>
                        </div>
                        <Link
                          href={workItemHref(item)}
                          className="mt-2 inline-block font-bold text-slate-900 hover:text-brand-700"
                        >
                          {item.title}
                        </Link>
                        ...
                        <p className="mt-2 text-[11px] text-slate-500">
                          ...
                          {item.assignedUserId === user.userId
                            ? 'you'
                            : item.assignedRole
                              ? item.assignedRole.replaceAll('_', ' ').toLowerCase()
                              : 'team queue'}{' '}
                          · Due {item.dueAt ? formatDate(item.dueAt) : 'without a configured target'}
