...
export default async function OperationsDashboardPage() {
  ...
  return <div className="flex min-h-screen flex-col bg-stone-50"><Header currentUser={user}/><main id="main-content" className="flex-1 py-8"><div className="page-shell space-y-7"><PageIntro eyebrow="Operations" title="Service checks" description="Overdue recruitment work, message delivery, scheduled jobs and candidates whose applications may need follow-up."/><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">{metrics.map(([label,value,href])=><Link key={label} href={href} className={`section-panel border-l-4 p-5 transition hover:bg-stone-50 ${value>0?'border-l-amber-500':'border-l-stone-300'}`}><p className="text-xs font-bold uppercase tracking-wide text-stone-500">{label}</p><p className="mt-2 text-3xl font-bold text-stone-950">{value}</p></Link>)}</div><section className="section-panel p-5"><h2 className="section-heading">Scheduled processing</h2><p className="mt-2 text-sm text-stone-600">Last run: {lastJob?`${lastJob.status.replaceAll('_', ' ').toLowerCase()} at ${lastJob.startedAt.toLocaleString()}`:'No run recorded'}</p></section><section className="section-panel p-5"><h2 className="section-heading">Applications with no activity for more than three days</h2><div className="mt-3 overflow-x-auto">{stuck.length===0?<EmptyState title="No stalled applications" description="All active applications have been updated within the last three days."/>:<table className="data-table"><thead><tr><th>Candidate</th><th>Vacancy</th><th>Stage</th><th>Owner</th><th>Last update</th></tr></thead><tbody>{stuck.map((item)=><tr key={item.id}><td><Link className="font-bold text-brand-700" href={`/recruitment/applications/${item.id}`}>{item.candidate.legalFirstName} {item.candidate.lastName}</Link></td><td>{item.vacancy.title}</td><td>{item.internalStatus.replace(/_/g,' ')}</td><td>{item.assignedReviewerId?'Assigned':'Unassigned'}</td><td>{item.updatedAt.toLocaleString()}</td></tr>)}</tbody></table>}</div></section></div></main><Footer/></div>
}
