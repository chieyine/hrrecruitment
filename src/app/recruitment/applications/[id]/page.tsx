...
export default function ApplicationReviewPage(props: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    fetch(`/api/recruitment/eligibility?applicationId=${params.id}`)
      .then((response) => response.json())
      .then((body) => setEligibility(body.evaluations?.[0] || null))
      .catch(() => undefined)
  }, [params.id])
  ...
  const decideEligibility = async (humanDecision: string, decisionReason: string) => {
    ...
    const response = await fetch('/api/recruitment/eligibility', {
      ...
      body: JSON.stringify({ action: 'DECIDE', evaluationId: eligibility.id, humanDecision, decisionReason }),
    })
    const body = await response.json()
    setMsg(response.ok ? 'Eligibility decision recorded.' : body.error || 'Eligibility decision failed')
    if (response.ok) {
  ...
  const handleScorecardSubmit = async (e: React.FormEvent) => {
    ...
    if (!conflictType) {
      setMsg('Complete the conflict-of-interest declaration first.')
      return
    }
    setSavingScore(true)
    try {
      ...
      if (!declaration.ok) {
        setMsg(declarationJson.error || 'Conflict declaration failed.')
        return
      }
  ...
  const outstanding = [
    ...
    ...(app.interviews || []).flatMap((interview: any) => {
      ...
      return missing
        ? [`${missing} panel score${missing === 1 ? '' : 's'} missing for ${interview.title || 'the interview'}.`]
        : []
    }),
    ...(app.offers || [])
      .filter(
        (offer: any) =>
          ...
          new Date(offer.acceptanceDeadline) < new Date(now + 48 * 60 * 60_000)
      )
      .map((offer: any) => `Offer response is due ${formatDateTime(offer.acceptanceDeadline)}.`),
    ...(app.preboardings || [])
      .filter(
        (item: any) =>
          item.readinessStatus !== 'READY_TO_RESUME' &&
          item.confirmedStartDate &&
          new Date(item.confirmedStartDate) < new Date(now + 7 * 86400000)
      )
      .map(() => 'Preboarding is not ready for the confirmed start date.'),
    ...(app.deliveryHistory || [])
      .filter((item: any) => ['FAILED', 'DEAD_LETTER'].includes(item.status))
      .map((item: any) => `Message delivery failed: ${item.subject || 'candidate notification'}.`),
  ]
  ...
  return (
    <div className="flex min-h-screen flex-col bg-slate-50">
        ...
        <div className="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8 space-y-8">
          ...
          {msg && (
            <div
              ...
              className={`rounded-xl border p-4 text-xs font-bold flex items-center gap-2 ${msgError ? 'border-rose-200 bg-rose-50 text-rose-800' : 'border-emerald-200 bg-emerald-50 text-emerald-800'}`}
            >
          ...
          {app?.possibleDuplicates?.length > 0 && (
            <div className="rounded-xl border border-amber-300 bg-amber-50 p-4 text-xs text-amber-900">
              <div className="flex items-center gap-2 font-bold">
                <ShieldAlert className="h-4 w-4" /> Possible duplicate candidate record
              </div>
              <p className="mt-1">
                A matching phone number appears on {app.possibleDuplicates.length} other profile
                {app.possibleDuplicates.length === 1 ? '' : 's'}. Verify identity before progressing; do not merge
          ...
          {/* Applicant Header */}
          <div className="rounded-2xl bg-white p-8 border border-slate-200 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span
                  className={`px-3 py-0.5 rounded-full text-xs font-bold border ${getStatusBadgeClass(app?.internalStatus)}`}
                >
            ...
            <div className="flex flex-wrap items-center gap-3">
              {app.capabilities?.exportDocumentation && (
                <>
                  ...
                  <a
                    href={`/api/recruitment/applications/${params.id}/documentation?includeFiles=1`}
              ...
              <Link href={`/recruitment/applications/${params.id}/handover`} className="btn-primary">
          ...
          <section className="section-panel" aria-labelledby="case-position-heading">
            <div className="section-heading">
              <div>
                <h2 id="case-position-heading" className="text-lg font-bold text-slate-950">
                  Case position
                </h2>
                <p className="mt-1 text-sm text-slate-600">
                  The current evidence and outstanding parts of this recruitment case.
                </p>
              </div>
            ...
            <dl className="grid gap-px overflow-hidden border border-slate-200 bg-slate-200 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                [
                  'Assessment',
                  app.candidateAssessments?.[0]
                    ? `${app.candidateAssessments[0].assessment.title}: ${app.candidateAssessments[0].status.replaceAll('_', ' ')}`
                    : 'Not started',
                ],
                [
                  'Interview',
                  app.interviews?.[0]
                    ? `${app.interviews[0].status.replaceAll('_', ' ')} · ${formatDate(app.interviews[0].scheduledStart)}`
                    : 'Not arranged',
                ],
                [
                  'References',
                  `${app.referenceStatus.replaceAll('_', ' ')} · ${app.referees?.filter((referee: any) => referee.requests?.some((request: any) => request.response)).length || 0}/${app.referees?.length || 0} received`,
                ],
                ['Offer', app.offers?.[0]?.status?.replaceAll('_', ' ') || 'Not created'],
                [
                  'Preboarding',
                  app.preboardings?.[0]
                    ? `${app.preboardings[0].overallCompletionPercentage}% · ${app.preboardings[0].readinessStatus.replaceAll('_', ' ')}`
                    : 'Not started',
                ],
              ].map(([label, value]) => (
            ...
            <div
              className={`mt-4 border p-4 ${outstanding.length ? 'border-amber-300 bg-amber-50 text-amber-950' : 'border-emerald-200 bg-emerald-50 text-emerald-900'}`}
            >
              <p className="text-sm font-bold">
                {outstanding.length
                  ? `${outstanding.length} item${outstanding.length === 1 ? '' : 's'} may block progress`
              ...
              {outstanding.length ? (
                ...
                <p className="mt-1 text-xs">
                  The available case evidence does not show an overdue or unresolved exception.
                </p>
              )}
            </div>
          </section>
            ...
            <div className="md:col-span-2 space-y-6">
              <div className="rounded-2xl bg-white p-6 border border-slate-200 shadow-sm space-y-4">
                <h3 className="text-xs font-bold uppercase tracking-wider text-slate-700 border-b border-slate-100 pb-2 flex items-center gap-2">
                  <User className="h-4 w-4 text-brand-600" /> Candidate Profile & Qualifications
                </h3>
                ...
                <div className="pt-2 space-y-2">
                  ...
                  {app?.candidate?.employment?.map((emp: any, i: number) => (
                    <div key={i} className="p-3 rounded-xl bg-slate-50 border border-slate-200 text-xs">
                      <strong>{emp.jobTitle}</strong> at {emp.employer} ({String(emp.startDate ?? '').slice(0, 4)} -{' '}
                      {emp.isCurrent || !emp.endDate ? 'Present' : String(emp.endDate).slice(0, 4)})
                    </div>
                  ))}
                </div>
                    ...
                    <dl className="mt-3 divide-y divide-slate-100">
                      {app.answers.map((answer: any) => (
                        <div key={answer.id} className="py-3">
                          <dt className="text-xs font-semibold text-slate-600">{answer.vacancyQuestion.label}</dt>
                          <dd className="mt-1 whitespace-pre-wrap text-sm text-slate-900">
                            {String(answer.answerJson).replace(/^"|"$/g, '')}
                          </dd>
                        </div>
                      ))}
                    </dl>
                  ) : (
                    <p className="mt-2 text-sm text-slate-500">No vacancy-specific answers were recorded.</p>
                  )}
                  <p className="mt-3 text-xs text-slate-500">
                    {app.files?.length || 0} application attachment{app.files?.length === 1 ? '' : 's'} recorded.
                  </p>
                </div>
              </div>
              ...
              <section className="section-panel" aria-labelledby="timeline-heading">
                <div className="section-heading">
                  <div>
                    <h3 id="timeline-heading" className="flex items-center gap-2 text-lg font-bold text-slate-950">
                      <Clock3 className="h-5 w-5 text-brand-700" />
                      Case timeline
                    </h3>
                    <p className="mt-1 text-sm text-slate-600">
                      Stage changes are recorded in chronological order for accountability.
                    </p>
                  </div>
                  <span className="status-chip bg-slate-100 text-slate-700">
                    {app.stageHistory?.length || 0} changes
                  </span>
                </div>
                {app.stageHistory?.length ? (
                  <ol className="relative ml-2 border-l border-slate-300">
                    {[...app.stageHistory].reverse().map((event: any) => (
                      <li key={event.id} className="relative ml-5 pb-5 last:pb-0">
                        <span className="absolute -left-[25px] top-1 h-2.5 w-2.5 rounded-full bg-brand-700 ring-4 ring-white" />
                  ...
                  <p className="text-sm text-slate-500">No stage changes have been recorded.</p>
                )}
                <div className="mt-5 border-t border-slate-200 pt-4 text-sm text-slate-600">
                  <MessageSquare className="mr-2 inline h-4 w-4" />
                  {app.messageThreads?.reduce((total: number, thread: any) => total + thread.messages.length, 0) ||
                    0}{' '}
                  candidate messages recorded.
                </div>
              </section>
              ...
              <section className="section-panel" aria-labelledby="communications-heading">
                <div className="section-heading">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-slate-600">
                      Messages, replies, read state and outbound delivery for this candidate.
                    </p>
                  </div>
                </div>
                <div className="space-y-4">
                  {(app.messageThreads || []).map((thread: any) => (
                    <article key={thread.id} className="border border-slate-200">
                      <div className="border-b border-slate-200 bg-slate-50 px-4 py-3">
                        <p className="text-sm font-bold text-slate-900">{thread.subject}</p>
                        <p className="mt-1 text-[11px] uppercase tracking-wide text-slate-500">
                          {thread.category.replaceAll('_', ' ')}
                          {thread.restricted ? ' · restricted' : ''}
                        </p>
                      </div>
                        ...
                        {thread.messages.map((message: any) => (
                          <div key={message.id} className="p-4">
                            <div className="flex flex-wrap justify-between gap-2 text-[11px] text-slate-500">
                              <span>{message.sender?.email || 'System user'}</span>
                              <span>
                                {formatDateTime(message.sentAt)} ·{' '}
                                {message.readAt ? `read ${formatDateTime(message.readAt)}` : 'not yet read'}
                              </span>
                            </div>
                  ...
                  {!app.messageThreads?.length && (
                    <p className="text-sm text-slate-500">No candidate messages have been recorded.</p>
                  )}
                  <div className="border-t border-slate-200 pt-4">
                    <MessageComposer applicationId={params.id} />
                  </div>
                  <details className="border border-slate-200 p-4">
                    <summary className="cursor-pointer text-sm font-bold text-slate-900">
                      Email delivery history ({app.deliveryHistory?.length || 0})
                    </summary>
                    <div className="mt-3 divide-y divide-slate-100">
                      {(app.deliveryHistory || []).map((delivery: any) => (
                        <div key={delivery.id} className="py-3 text-xs">
                          <div className="flex justify-between gap-3">
                            <span className="font-semibold text-slate-900">
                              {delivery.subject || 'Candidate notification'}
                            </span>
                            <span
                              className={
                          ...
                          <p className="mt-1 text-slate-500">
                            {formatDateTime(delivery.createdAt)} · {delivery.attempts} attempt
              ...
              <section className="section-panel" aria-labelledby="governance-heading">
                <div className="section-heading">
                  <div>
                    <h3 id="governance-heading" className="flex items-center gap-2 text-lg font-bold text-slate-950">
                      <ClipboardCheck className="h-5 w-5 text-brand-700" />
                      Decisions, notes and audit
                    </h3>
                    <p className="mt-1 text-sm text-slate-600">The accountable record behind this case.</p>
                  </div>
                </div>
                <div className="grid gap-5 lg:grid-cols-2">
                  <div>
                    <h4 className="text-sm font-bold">Approval decisions</h4>
                    {app.approvals?.length ? (
                      <div className="mt-2 divide-y divide-slate-100">
                        {app.approvals.map((approval: any) => (
                          <div key={approval.id} className="py-3 text-xs">
                            <p className="font-semibold">
                              {approval.resourceType} · stage {approval.stage} · {approval.decision}
                            </p>
                            <p className="mt-1 text-slate-500">
                      ...
                      <p className="mt-2 text-xs text-slate-500">No approval decisions recorded.</p>
                    )}
                  </div>
                  <div>
                    <h4 className="text-sm font-bold">Case notes</h4>
                    {app.notes?.length ? (
                      <div className="mt-2 divide-y divide-slate-100">
                        {app.notes.map((note: any) => (
                          <div key={note.id} className="py-3 text-xs">
                            <p className="font-semibold">
                              {note.category.replaceAll('_', ' ')}
                              {note.restricted ? ' · restricted' : ''}
                            </p>
                            <p className="mt-1 whitespace-pre-wrap text-slate-700">{note.content}</p>
                            <p className="mt-1 text-slate-500">{formatDateTime(note.createdAt)}</p>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="mt-2 text-xs text-slate-500">No case notes recorded.</p>
                    )}
                  </div>
                </div>
                <div className="mt-5 border-t border-slate-200 pt-5">
                  <CaseGovernanceActions
                    ...
                    scorecards={(app.scorecards || []).map((scorecard: any) => ({
                      id: scorecard.id,
                ...
                <details className="mt-5 border border-slate-200 p-4">
                  <summary className="flex cursor-pointer items-center gap-2 text-sm font-bold">
                    ...
                    Audit history ({app.auditHistory?.length || 0})
                  </summary>
                  <ol className="mt-3 divide-y divide-slate-100">
                    {(app.auditHistory || []).map((entry: any) => (
                      <li key={entry.id} className="py-3 text-xs">
                        <p className="font-semibold text-slate-900">{entry.action.replaceAll('_', ' ')}</p>
                        <p className="mt-1 text-slate-500">
                          {entry.resourceType} · {formatDateTime(entry.createdAt)}
                  ...
                  <form onSubmit={handleScorecardSubmit} className="space-y-4 text-xs">
                    <div className="rounded-xl border border-amber-200 bg-amber-50 p-3 space-y-2">
                      ...
                      <select
                        ...
                        className="w-full rounded-lg border border-amber-300 bg-white p-2"
                      >
                      ...
                      {conflictType && conflictType !== 'NONE' && (
                        <textarea
                          ...
                          placeholder="Describe the conflict. Scoring will remain blocked until HR resolves it."
                          className="w-full rounded-lg border border-amber-300 p-2"
                        />
                      )}
                    </div>
                    {criteria.map((c) => (
                    ...
                    <button
                      ...
                      className="w-full rounded-xl bg-brand-600 py-2.5 text-xs font-bold text-white shadow hover:bg-brand-700 transition-all"
                    >
            ...
            <div className="space-y-6">
              {eligibility && (
                <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
                  <h3 className="text-xs font-bold uppercase text-slate-700">Objective eligibility review</h3>
                  <p className="mt-2 text-sm font-bold">
                    System suggestion: {eligibility.suggestedOutcome.replace(/_/g, ' ')}
                  </p>
                  <p className="mt-1 text-xs text-slate-500">
                    This is decision support only. A staff member must confirm the outcome and reason.
                  </p>
                  {!eligibility.humanDecision ? (
                    <div className="mt-3 flex flex-wrap gap-2">
                      <button
                        onClick={() => setDecidingEligibility('ELIGIBLE')}
                        className="rounded bg-emerald-700 px-3 py-2 text-xs font-bold text-white"
                      >
                        Confirm eligible
                      </button>
                      <button
                        onClick={() => setDecidingEligibility('INELIGIBLE')}
                        className="rounded bg-rose-700 px-3 py-2 text-xs font-bold text-white"
                      >
                        Confirm ineligible
                      </button>
                      <button
                        onClick={() => setDecidingEligibility('NEEDS_MORE_INFORMATION')}
                    ...
                    <p className="mt-3 rounded bg-brand-50 p-2 text-xs">
                      <b>Human decision:</b> {eligibility.humanDecision} — {eligibility.decisionReason}
                    </p>
                  )}
                </div>
              )}
                ...
                <form onSubmit={handleStageChange} className="space-y-3 text-xs">
                  <div>
                    ...
                    <select
                      ...
                      className="w-full rounded-xl border border-slate-300 p-2 text-xs bg-white focus:border-brand-600 focus:outline-none"
                    >
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full rounded-xl border border-slate-300 p-2 text-xs focus:border-brand-600 focus:outline-none"
                    />
                  </div>
              ...
              <div className="rounded-2xl bg-amber-50 border border-amber-200 p-4 text-xs text-amber-900 space-y-1">
                ...
                <p>ERP record creation occurs strictly after actual physical/remote resumption confirmation.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <ReasonDialog
        ...
        onConfirm={(reason) => {
          if (decidingEligibility) return decideEligibility(decidingEligibility, reason)
        }}
