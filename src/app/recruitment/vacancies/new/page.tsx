...
export default function NewVacancyPage() {
  ...
  useEffect(() => {
    fetch('/api/recruitment/vacancies')
      ...
      .then((data) => { setDepartments(data.departments || []); setDutyStations(data.dutyStations || []); setCategories(data.categories || []); setProjects(data.projects || []); setScorecards(data.scorecards || []); setPackages(data.packages || []) })
          ...
          <div className="rounded-3xl bg-white p-8 border border-slate-200 shadow-sm space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 space-y-3">
                ...
                {requiredDocuments.map((document, index) => <div key={index} className="flex gap-2"><input required value={document.documentType} onChange={(e) => setRequiredDocuments(requiredDocuments.map((item, i) => i === index ? { ...item, documentType: e.target.value } : item))} placeholder="e.g. CV" className="flex-1 rounded-lg border border-slate-300 p-2 text-xs" /><button type="button" onClick={() => setRequiredDocuments(requiredDocuments.filter((_, i) => i !== index))} className="text-xs font-bold text-rose-700">Remove</button></div>)}
              </div>
