...
export default async function RecruitmentInsightsPage() {
  ...
  const [
    ...
    prisma.bulkActionRun.findMany({
      where: { createdAt: { gte: windowStart } },
      orderBy: { createdAt: 'desc' },
      take: 2000,
    }),
    // Counting is the database's job; this used to return one row per application.
    prisma.auditLog.groupBy({
      by: ['resourceId'],
      where: { resourceType: 'Application', createdAt: { gte: windowStart } },
      _count: true,
    }),
  ])
  ...
  const withdrawalRate = applications.length
    ? (applications.filter((item) => item.internalStatus === 'WITHDRAWN').length / applications.length) * 100
    : 0
  ...
  const awaitingManager = openWork
    .filter((item) => item.assignedRole === 'HIRING_MANAGER')
    .map((item) => item.createdAt.getTime())
  const preboardingBeforeStart = preboardings.filter(
    (item) =>
      item.confirmedStartDate &&
      item.readinessStatus === 'READY_TO_RESUME' &&
      Boolean(item.readyAt && item.readyAt <= item.confirmedStartDate)
  ).length
  const abandonment = applications.filter(
    (item) => item.internalStatus === 'DRAFT' && item.updatedAt < new Date(now.getTime() - 3 * 86400000)
  ).length
  ...
  const metrics = [
    [
      ...
      'Median elapsed time for applications that reached shortlist.',
      '/recruitment/applications',
    ],
    [
      'HR touches per application',
      applications.length ? (touches / applications.length).toFixed(1) : '0',
      'Direct application audit actions divided by applications.',
      '/recruitment/audit',
    ],
    [
      'Overdue work items',
      String(openWork.filter((item) => item.dueAt && item.dueAt < now).length),
      'Open work beyond its service target.',
      '/recruitment/work?attention=overdue',
    ],
    [
      'Awaiting hiring manager',
      awaitingManager.length ? `${days(median(awaitingManager.map((value) => now.getTime() - value)))} days` : '0 days',
      'Median age of work assigned to hiring managers.',
      '/recruitment/work?scope=team',
    ],
    [
      'Candidate support cases',
      String(supportRequests),
      'Candidate complaints, appeals, accommodations and other support cases.',
      '/recruitment/complaints',
    ],
    [
      'Message delivery failures',
      String(deliveryFailures),
      'Failed or dead-letter outbound messages.',
      '/admin/system-settings',
    ],
    [
      'Application abandonment',
      String(abandonment),
      'Drafts inactive for more than three days.',
      '/recruitment/applications',
    ],
    [
      'Ready before start date',
      `${preboardings.length ? Math.round((preboardingBeforeStart / preboardings.length) * 100) : 0}%`,
      'Preboarding cases ready on or before the confirmed start date.',
      '/recruitment/preboarding',
    ],
    [
      'Automated actions',
      String(automationActions),
      'Successfully completed automation actions retained in the action log.',
      '/admin/automations',
    ],
    [
      'Bulk action error rate',
      `${totalBulk ? ((bulkFailures / totalBulk) * 100).toFixed(1) : '0.0'}%`,
      'Per-record failures across recorded bulk operations.',
      '/recruitment/applications',
    ],
  ] as const
      ...
      <main id="main-content" className="flex-1 py-8">
        <div className="page-shell space-y-7">
          <div className="page-intro">
            <p className="editorial-kicker">Recruitment reporting</p>
            <h1 className="page-title">Decisions and workload</h1>
            <p className="mt-2 max-w-3xl text-sm text-slate-600">
              Review delays, recruitment outcomes and the volume of manual follow-up.
            </p>
          </div>
          <section className="section-panel">
            <h2 className="text-lg font-bold">Recruitment outcomes</h2>
            <div className="mt-4 grid gap-px border border-slate-200 bg-slate-200 sm:grid-cols-2 lg:grid-cols-5">
              {metrics.map(([label, value, description, href]) => (
                <Link key={label} href={href} className="bg-white p-4 hover:bg-slate-50">
                  <span className="text-xs font-semibold text-slate-600">{label}</span>
                  <span className="mt-1 block text-2xl font-bold text-slate-950">{value}</span>
                  <span className="mt-2 block text-[11px] leading-4 text-slate-500">{description}</span>
                </Link>
              ))}
            </div>
          </section>
          <div className="grid gap-6 lg:grid-cols-2">
            <section className="section-panel">
              <h2 className="font-bold">Where candidates are getting stuck</h2>
              <p className="mt-1 text-xs text-slate-500">Median time spent after entering each stage.</p>
            ...
            <section className="section-panel">
              <h2 className="font-bold">Vacancies taking longest</h2>
              <table className="data-table mt-3">
                <thead>
                  <tr>
                    <th>Vacancy</th>
                ...
                <tbody>
                  {vacancyRows.slice(0, 12).map((row) => (
                    <tr key={row.id}>
                      <td>
                        <Link
            ...
            <section className="section-panel">
              <h2 className="font-bold">Panel scoring consistency</h2>
              <table className="data-table mt-3">
                <thead>
                  <tr>
                    <th>Vacancy</th>
            ...
            <section className="section-panel">
              <h2 className="font-bold">Offer declines</h2>
              <p className="mt-1 text-xs text-slate-500">
                {withdrawalRate.toFixed(1)}% candidate withdrawal rate across all recorded applications.
              </p>
              <table className="data-table mt-3">
                <thead>
                  <tr>
            ...
            <section className="section-panel">
              <h2 className="font-bold">New starters at risk</h2>
              <p className="mt-2 text-3xl font-bold">{atRisk.length}</p>
              <p className="mt-1 text-sm text-slate-600">Start within seven days and not ready to resume.</p>
              <Link
                href="/recruitment/preboarding"
                className="mt-3 inline-block text-sm font-semibold text-brand-700 underline"
              >
                Open underlying preboarding cases
              </Link>
            </section>
            <section className="section-panel">
              <h2 className="font-bold">Outcomes by permitted operational dimension</h2>
              <p className="mt-1 text-xs text-slate-500">
                Department is shown as an operational dimension. Protected candidate characteristics are excluded from
                this view.
              </p>
              <table className="data-table mt-3">
                <thead>
