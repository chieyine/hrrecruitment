...
export default async function RecruitmentInsightsPage() {
  ...
  const [applications, vacancies, submissions, offers, preboardings, openWork, deliveryFailures, supportRequests, automationActions, bulkRuns, auditTouches] = await Promise.all([
    ...
    prisma.vacancy.findMany({ where: { createdAt: { gte: windowStart } }, include: { department: true, applications: { select: { id: true, internalStatus: true, submittedAt: true, updatedAt: true } } }, orderBy: { createdAt: 'desc' }, take: 2000 }),
    prisma.interviewPanelSubmission.findMany({ where: { submittedAt: { gte: windowStart } }, include: { interview: { include: { application: { include: { vacancy: true } } } } }, take: 5000 }),
    prisma.offer.findMany({ where: { startDate: { gte: windowStart } }, include: { application: { include: { vacancy: true } } }, take: 5000 }),
    prisma.candidatePreboarding.findMany({ where: { startedAt: { gte: windowStart } }, include: { application: { include: { vacancy: true } } }, take: 5000 }),
    ...
    prisma.bulkActionRun.findMany({ where: { createdAt: { gte: windowStart } }, orderBy: { createdAt: 'desc' }, take: 2000 }),
    // Counting is the database's job; this used to return one row per application.
    prisma.auditLog.groupBy({ by: ['resourceId'], where: { resourceType: 'Application', createdAt: { gte: windowStart } }, _count: true }),
  ])
  ...
  return <div className="flex min-h-screen flex-col bg-stone-50"><Header currentUser={user}/><main id="main-content" className="flex-1 py-8"><div className="page-shell space-y-7">
    <div className="page-intro"><p className="editorial-kicker">Recruitment reporting</p><h1 className="page-title">Decisions and workload</h1><p className="mt-2 max-w-3xl text-sm text-slate-600">Review delays, recruitment outcomes and the volume of manual follow-up.</p></div>
    <section className="section-panel"><h2 className="text-lg font-bold">Recruitment outcomes</h2><div className="mt-4 grid gap-px border border-slate-200 bg-slate-200 sm:grid-cols-2 lg:grid-cols-5">{metrics.map(([label,value,description,href])=><Link key={label} href={href} className="bg-white p-4 hover:bg-slate-50"><span className="text-xs font-semibold text-slate-600">{label}</span><span className="mt-1 block text-2xl font-bold text-slate-950">{value}</span><span className="mt-2 block text-[11px] leading-4 text-slate-500">{description}</span></Link>)}</div></section>
