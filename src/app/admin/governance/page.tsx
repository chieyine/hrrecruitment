    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-slate-600">
          Legal holds, retention evidence, access certification, audit integrity and failed delivery recovery.
        </p>
      </div>
      {message && (
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        <div className="border bg-white p-4">
          ...
          <p className={`mt-2 font-bold ${data.auditIntegrity.valid ? 'text-emerald-700' : 'text-rose-700'}`}>
            {data.auditIntegrity.valid ? 'Valid' : 'Integrity issue'}
          </p>
          <p className="text-xs">{data.auditIntegrity.checked || 0} chained entries</p>
        </div>
        <button
          disabled={busy}
          onClick={() => void action({ action: 'RUN_RETENTION' })}
          className="border bg-white p-4 text-left disabled:opacity-50"
        >
        ...
        <button
          disabled={busy}
          onClick={() => void action({ action: 'RETRY_DEAD_LETTERS' })}
          className="border bg-white p-4 text-left disabled:opacity-50"
        >
      ...
      <section className="border bg-white p-4">
        ...
        {data.legalHolds.map((hold: any) => (
          <div key={hold.id} className="mt-3 flex justify-between gap-3 border-t pt-3 text-sm">
            <span>
              <b>{hold.resourceType}</b> {hold.resourceId}
              <span className="block text-xs text-slate-500">
                {hold.reason} · {hold.status}
              </span>
            </span>
            {hold.status === 'ACTIVE' && (
              <button onClick={() => setReleaseHold(hold)} className="text-xs font-bold text-rose-700">
                Release
              </button>
            )}
          </div>
      ...
      <section className="border bg-white p-4">
        ...
        {data.accessReviews.map((review: any) => (
          <div key={review.id} className="mt-3 flex justify-between gap-3 border-t pt-3 text-xs">
            <span>
              User {review.userId} · reviewer {review.reviewerUserId} · due{' '}
              {formatDate(review.dueAt)} · <b>{review.status}</b>
            </span>
            {review.status === 'PENDING' && (
              <button onClick={() => setReviewDecision(review)} className="font-bold text-emerald-700">
                Record decision
              </button>
            )}
          </div>
      ...
      <section className="border bg-white p-4">
        ...
        {data.retentionRuns.map((run: any) => (
          <p key={run.id} className="mt-2 border-t pt-2 text-xs">
            {formatDateTime(run.startedAt)} · {run.status} · {run.evidenceHash || 'No evidence hash'}
          </p>
        ))}
      </section>
      <section className="border bg-white p-4">
        ...
        {data.operationalEvents.length === 0 && (
          <p className="mt-3 text-sm text-slate-500">No unresolved operational events.</p>
        )}
        {data.operationalEvents.map((event: any) => (
          <p key={event.id} className="mt-2 border-t pt-2 text-xs">
            <b>{event.severity}</b> {event.eventType} · {event.resourceType} {event.resourceId}
          </p>
        ))}
      </section>
        ...
        >
          <label className="block text-xs font-semibold">
            ...
            >
              ...
              <option>REFERENCE_REQUEST</option>
              <option>OUTBOX_MESSAGE</option>
              <option>IDEMPOTENCY_RECORD</option>
              <option>RATE_LIMIT_BUCKET</option>
            </select>
          </label>
          <label className="block text-xs font-semibold">
            Resource ID
            <input
              ...
              onChange={(event) => setHoldForm({ ...holdForm, resourceId: event.target.value })}
              className="mt-1 w-full rounded border p-2.5 text-sm"
            />
          </label>
          <label className="block text-xs font-semibold">
            ...
            <textarea
              ...
              onChange={(event) => setHoldForm({ ...holdForm, reason: event.target.value })}
              className="mt-1 w-full rounded border p-2.5 text-sm"
            />
          </label>
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setHoldOpen(false)} className="rounded border px-4 py-2 text-sm">
              Cancel
            </button>
        ...
        >
          <label className="block text-xs font-semibold">
            ...
            <select
              ...
              onChange={(event) => setReviewForm({ ...reviewForm, userId: event.target.value })}
              className="mt-1 w-full rounded border p-2.5 text-sm"
            >
          ...
          <label className="block text-xs font-semibold">
            ...
            <select
              ...
              onChange={(event) => setReviewForm({ ...reviewForm, reviewerUserId: event.target.value })}
              className="mt-1 w-full rounded border p-2.5 text-sm"
            >
          ...
          <label className="block text-xs font-semibold">
            ...
            <input
              type="date"
              required
              value={reviewForm.dueAt}
              onChange={(event) => setReviewForm({ ...reviewForm, dueAt: event.target.value })}
              className="mt-1 w-full rounded border p-2.5 text-sm"
            />
          </label>
          <div className="flex justify-end gap-2">
            <button type="button" onClick={() => setReviewOpen(false)} className="rounded border px-4 py-2 text-sm">
              Cancel
            </button>
            <button disabled={busy} className="rounded bg-brand-700 px-4 py-2 text-sm font-semibold text-white">
              Create review
      ...
      <ReasonDialog
        ...
        onConfirm={async (reason) => {
          if (await action({ action: 'RELEASE_HOLD', id: releaseHold.id, reason })) setReleaseHold(null)
        }}
        title="Release legal hold"
        description="Explain why the hold no longer applies."
      ...
      <ReasonDialog
        ...
        onConfirm={async (decisionComment) => {
          if (
            await action({ action: 'DECIDE_ACCESS_REVIEW', id: reviewDecision.id, status: 'APPROVED', decisionComment })
          )
            setReviewDecision(null)
        }}
        title="Certify account access"
        description="Record the evidence reviewed before certifying access."
        confirmLabel="Certify access"
        reasonLabel="Evidence and decision"
        reasonRequired
        busy={busy}
      />
    </div>
