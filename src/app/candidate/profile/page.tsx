          ...
          <div className="rounded-3xl bg-white p-8 border border-slate-200 shadow-sm space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-6">
              <div>
                ...
                <p className="text-xs text-slate-500">{user.email} • {profile?.primaryPhone || 'No phone set'}</p>
              </div>

              <div className="flex flex-col items-end gap-3">
                <Link href="/candidate/profile/personal" className="inline-flex items-center rounded-xl bg-blue-700 px-4 py-2.5 text-xs font-bold text-white hover:bg-blue-800">
                ...
                {completion.missing.length > 0 && <span className="block max-w-xs text-slate-500">Still to add: {completion.missing.slice(0, 3).join(', ')}</span>}
                </div>
              </div>
            </div>
            ...
            <nav aria-label="Edit profile sections" className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
              {[
                ...
                <Link key={href} href={href} className="flex items-center justify-between rounded-xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-bold text-slate-800 hover:border-blue-300 hover:bg-blue-50">
                  <span className="flex items-center gap-2"><Icon className="h-4 w-4 text-blue-700" />{label}</span>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-xs">
                ...
                <div>
                  ...
                  <span className="font-bold text-slate-900">{profile?.state || 'N/A'} - {profile?.lga || ''}</span>
                </div>
                <div>
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                ...
                <Link href="/candidate/profile/education" className="text-xs font-bold text-blue-700 hover:underline">Edit education</Link>
              </div>
                ...
                <div className="space-y-3">
                  {profile?.education.map((edu) => (
                    <div key={edu.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs space-y-1">
                      <h4 className="font-bold text-slate-900 text-sm">{edu.qualification} in {edu.fieldOfStudy}</h4>
                      <p className="text-slate-600 font-medium">{edu.institution} ({edu.country})</p>
                      <p className="text-slate-400">{edu.startYear} - {edu.completionYear} {edu.grade ? `• Grade: ${edu.grade}` : ''}</p>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                ...
                <Link href="/candidate/profile/employment" className="text-xs font-bold text-blue-700 hover:underline">Edit employment</Link>
              </div>
                ...
                <div className="space-y-3">
                  {profile?.employment.map((emp) => (
                    <div key={emp.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs space-y-1">
                      ...
                      <p className="text-slate-600 font-medium">{emp.employer} ({emp.country})</p>
                      <p className="text-slate-400">
                        {new Date(emp.startDate).getFullYear()} - {emp.isCurrent ? 'Present' : emp.endDate ? new Date(emp.endDate).getFullYear() : ''}
                      </p>
            ...
            <div className="space-y-4 pt-2">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                ...
                <Link href="/candidate/profile/licences" className="text-xs font-bold text-blue-700 hover:underline">Edit licences</Link>
              </div>
                ...
                <div className="space-y-3">
                  {profile?.licences.map((lic) => (
                    <div key={lic.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs space-y-1">
                      <h4 className="font-bold text-slate-900 text-sm">{lic.licenceType} - {lic.professionalBody}</h4>
