...
export default async function CandidateTasksPage() {
  ...
  const [
    ...
    prisma.candidatePreboarding.findMany({
      where: {
        ...
        application: { offers: { some: { status: 'ACCEPTED' } }, candidate: { userId: user.userId } },
      },
      select: {
        id: true,
        application: {
  ...
  const items: CandidateTask[] = [
    ...
    ...offers.map((item) => ({
      key: `offer-${item.id}`,
      title: `Respond to offer: ${item.position}`,
      context: 'Offer decision',
    ...
    ...documents.map((item) => ({
      ...
      title: `Submit document: ${item.documentRequirement.name}`,
      context: 'Preboarding',
        ...
        <div className="mx-auto max-w-4xl space-y-6 px-4">
          <div className="border-b border-slate-300 pb-5">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Assessments, interview replies, offers and preboarding items that need your attention.
            </p>
          </div>
          {overdue > 0 && (
            <div className="flex items-center gap-2 rounded-2xl border border-amber-300 bg-amber-50 p-4 text-sm font-semibold text-amber-900">
              <AlertCircle className="h-5 w-5" />
              {overdue} item{overdue === 1 ? ' is' : 's are'} overdue. Contact HR if you need help or an adjustment.
            </div>
          )}
          <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            {items.length === 0 ? (
              <div className="p-12 text-center">
                ...
                <h2 className="mt-3 font-bold">No outstanding tasks</h2>
                <p className="mt-1 text-sm text-slate-500">We will list new tasks here when they are assigned.</p>
              </div>
            ) : (
                ...
                {items.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div
                        className={`shrink-0 text-right text-xs font-bold ${isOverdue ? 'text-red-700' : 'text-slate-600'}`}
                      >
                        <Clock3 className="mb-1 ml-auto h-4 w-4" />
                        {item.dueAt ? `${isOverdue ? 'Overdue · ' : ''}${formatDateTime(item.dueAt)}` : 'No deadline'}
                      </div>
                    </Link>
                  )
                })}
          ...
          >
            Request an adjustment or accommodation
          </Link>
        </div>
      </main>
      <Footer />
