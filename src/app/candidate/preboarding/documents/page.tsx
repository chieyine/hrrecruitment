                ...
                <div key={d.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs">
                  <div className="flex items-center justify-between gap-4">
                    <span className="font-bold text-slate-900">
                      {d.documentRequirement?.name || 'Document'}{' '}
                      {!d.required && <span className="font-normal text-slate-500">(optional)</span>}
                    </span>
                    <span className={`px-3 py-1 rounded-full font-bold border ${getStatusBadgeClass(d.status)}`}>
                      {d.status.replace(/_/g, ' ')}
                    </span>
                  </div>
                  {d.dueAt && <p className="mt-2 text-slate-600">Due {formatDate(d.dueAt)}</p>}
                  {d.rejectionReason && (
                    <p role="alert" className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-amber-900">
                      HR requested a replacement: {d.rejectionReason}
                    </p>
                  )}
                  {!['APPROVED', 'WAIVED'].includes(d.status) && (
                    <DocumentAction
                      resourceId={d.id}
                      sensitivityClass={d.documentRequirement?.sensitivityClass}
                      expiryRequired={d.documentRequirement?.expiryRequired}
                      allowedFileTypes={d.documentRequirement?.allowedFileTypes}
                    />
                  )}
                </div>
              ))
