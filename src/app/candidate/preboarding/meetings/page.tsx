              ...
              meetings.map((m) => (
                <div key={m.id} className="p-4 rounded-2xl bg-slate-50 border border-slate-200 text-xs space-y-1">
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`px-3 py-1 rounded-full font-bold border ${getStatusBadgeClass(m.status)}`}>
                      {m.status}
                    </span>
                  </div>
                  <p className="text-slate-500">
                    {formatDateTime(m.scheduledStart)} • {m.meetingLink || m.venue || 'TBC'}
                  </p>
                  {!m.candidateResponse && (
                    <div className="flex flex-wrap gap-2">
                      <SimpleAction
                        resourceId={m.id}
                        action="MEETING_RESPOND"
