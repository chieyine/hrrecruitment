        ...
        <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 space-y-8">
          <div className="rounded-2xl bg-slate-900 p-8 text-white shadow-xl space-y-2">
            ...
            <p className="text-xs text-slate-300">
              FRAD Human Resources verification for <strong>{candidateName}</strong> applying for{' '}
              <strong>{position}</strong>.
              {invalidLink && (
                <span
                  role="alert"
                  className="mt-2 block rounded-lg bg-rose-50 border border-rose-200 p-2 text-rose-700"
                >
                  {invalidLink}
                </span>
              )}
          ...
          {submitted ? (
            <div className="rounded-2xl bg-white p-8 border border-slate-200 shadow-sm text-center space-y-4">
              ...
              <p className="text-xs text-slate-500 max-w-md mx-auto">
                Thank you for providing your professional evaluation. Your response has been securely logged with FRAD
                Human Resources.
              </p>
            </div>
          ) : (
            <div className="rounded-2xl bg-white p-8 border border-slate-200 shadow-sm space-y-6">
              ...
              <form onSubmit={handleSubmit} className="space-y-5 text-xs">
                <div>
                  <label className="block font-bold text-slate-900 mb-1">1. Confirm Employment Dates & Role</label>
                  <input
                    type="text"
                    required
                    value={confirmDates}
                ...
                <div>
                  <label className="block font-bold text-slate-900 mb-1">2. Quality of Performance & Reliability</label>
                  <select
                    value={workQuality}
                ...
                <label className="block font-bold text-slate-900">
                  Main responsibilities held by the candidate
                  <textarea
                    required
                    rows={3}
                ...
                <div>
                  <label className="block font-bold text-slate-900 mb-1">3. Eligibility for Re-employment</label>
                  <select
                    value={rehire}
                ...
                <div>
                  <label className="block font-bold text-slate-900 mb-1">5. Overall Reference Recommendation</label>
                  <select
                    value={outcome}
                    onChange={(e) => setOutcome(e.target.value)}
                    className="w-full rounded-xl border border-slate-300 p-2.5 text-xs bg-white focus:border-brand-600 focus:outline-none font-bold"
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="Add any useful context about integrity, leadership or technical skill."
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex items-center justify-center gap-2 rounded-xl bg-brand-600 py-3 text-sm font-bold text-white shadow-lg hover:bg-brand-700 disabled:opacity-50 transition-all w-full"
                >
