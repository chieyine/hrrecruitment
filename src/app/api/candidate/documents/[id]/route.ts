...
export async function PATCH(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const document = await prisma.candidateDocument.findFirst({ where: { id: params.id, candidate: { userId: user.userId } } })
    ...
    await logAudit({ actorUserId: user.userId, action: 'DOCUMENT_METADATA_UPDATED', resourceType: 'CandidateDocument', resourceId: document.id })
...
export async function DELETE(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await logAudit({ actorUserId: user.userId, action: 'DOCUMENT_DELETED', resourceType: 'CandidateDocument', resourceId: document.id })
