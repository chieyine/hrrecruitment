...
export async function POST(_: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (['SUBMITTED', 'AUTO_SUBMITTED', 'MARKED', 'PASSED', 'FAILED'].includes(record.status)) throw new AuthzError('Assessment has already been submitted', 409)
    const now = new Date()
    if (record.assessment.opensAt && record.assessment.opensAt > now) throw new AuthzError('Assessment is not open yet', 409)
    if (record.assessment.closesAt && record.assessment.closesAt <= now) throw new AuthzError('Assessment has closed', 409)
    ...
    if (['INVITED', 'NOT_STARTED'].includes(record.status)) {
      ...
      await logAudit({ actorUserId: user.userId, action: 'ASSESSMENT_STARTED', resourceType: 'CandidateAssessment', resourceId: record.id })
    }
    if (record.status !== 'IN_PROGRESS' && !['INVITED', 'NOT_STARTED'].includes(record.status)) throw new AuthzError('Assessment cannot be started', 409)
    ...
    const savedAnswers = Object.fromEntries(record.answers.map((answer) => {
      try { return [answer.assessmentQuestionId, answer.answerJson ? JSON.parse(answer.answerJson) : ''] }
      catch { return [answer.assessmentQuestionId, answer.answerJson ?? ''] }
    }))
    ...
    return Response.json({ assessment: {
      id: record.id, title: record.assessment.title, description: record.assessment.description,
      status: 'IN_PROGRESS', secondsRemaining, savedAnswers,
      questions: questions.map((question) => ({ id: question.id, questionType: question.questionType, prompt: question.prompt, optionsJson: question.optionsJson, maximumScore: question.maximumScore })),
    } })
