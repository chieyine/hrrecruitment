...
const schema = z.union([
  ...
  z.object({
    ...
    speakingLevel: z.enum(['BASIC', 'INTERMEDIATE', 'FLUENT', 'NATIVE']),
    readingLevel: z.enum(['BASIC', 'INTERMEDIATE', 'FLUENT', 'NATIVE']),
    writingLevel: z.enum(['BASIC', 'INTERMEDIATE', 'FLUENT', 'NATIVE']),
  }),
  z
    .object({
    ...
    .refine((value) => !value.expiryDate || !value.issueDate || value.expiryDate >= value.issueDate, {
      path: ['expiryDate'],
...
export async function POST(request: Request) {
  try {
    ...
    if (input.kind === 'CERTIFICATION') await assertOwnedCleanFile(input.fileId, user.userId)
    let item: { id: string }
    if (input.kind === 'SKILL')
      item = await prisma.candidateSkill.create({
        data: {
      ...
      item = await prisma.candidateLanguage.create({
        data: {
          candidateId: profile.id,
      ...
      item = await prisma.candidateCertification.create({
        data: {
          candidateId: profile.id,
          name: input.name,
    ...
    await logAudit({
      ...
      action: `PROFILE_${input.kind}_ADDED`,
...
export async function PATCH(request: Request) {
  try {
    ...
    if (input.kind === 'SKILL') {
      ...
      count = (
        await prisma.candidateLanguage.updateMany({
          where: { id: input.id, candidateId: profile.id },
          data: {
            language: input.language,
            speakingLevel: input.speakingLevel,
      ...
      count = (
        await prisma.candidateCertification.updateMany({
          where: { id: input.id, candidateId: profile.id },
          data: {
            name: input.name,
            issuingBody: input.issuingBody,
    ...
    await logAudit({
      actorUserId: user.userId,
      action: `PROFILE_${input.kind}_UPDATED`,
      resourceType: 'CandidateProfile',
      resourceId: profile.id,
    })
...
export async function DELETE(request: Request) {
  try {
    const user = await requireUser()
    const { kind, id } = await parseBody(
      request,
      z.object({ kind: z.enum(['SKILL', 'LANGUAGE', 'CERTIFICATION']), id: z.string().min(1) })
    )
    const profile = await prisma.candidateProfile.findUnique({ where: { userId: user.userId } })
    if (!profile) throw new AuthzError('Profile not found', 404)
    let count = 0
    if (kind === 'SKILL')
      count = (await prisma.candidateSkill.deleteMany({ where: { id, candidateId: profile.id } })).count
    else if (kind === 'LANGUAGE')
      count = (await prisma.candidateLanguage.deleteMany({ where: { id, candidateId: profile.id } })).count
    else count = (await prisma.candidateCertification.deleteMany({ where: { id, candidateId: profile.id } })).count
    if (!count) throw new AuthzError('Profile item not found', 404)
    await logAudit({
      actorUserId: user.userId,
      action: `PROFILE_${kind}_DELETED`,
      resourceType: 'CandidateProfile',
      resourceId: profile.id,
    })
    return Response.json({ success: true })
