...
const messageSchema = z
  ...
  .refine((value) => Boolean(value.threadId || value.applicationId), 'threadId or applicationId is required')
...
export async function GET(request: Request) {
  try {
    ...
    const thread = threadId
      ? await prisma.messageThread.findUnique({ where: { id: threadId }, select: { applicationId: true } })
      : null
...
export async function POST(request: Request) {
  try {
    ...
    if (threadId) {
      ...
      if (thread.restricted && (!isStaff || !(await hasPermission(user.userId, 'preboarding.restricted.read'))))
        throw new AuthzError('Restricted thread permission is required', 403)
