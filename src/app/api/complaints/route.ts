...
export async function POST(request: Request) {
  try {
    ...
    const created = await prisma.complaintCase.create({
      data: {
        referenceNumber,
        reporterUserId: user?.userId || null,
        reporterEmail: input.reporterEmail || user?.email || null,
        applicationId: input.applicationId || null,
        category: input.category,
        subject: input.subject,
        description: input.description,
        priority: input.category === 'SAFEGUARDING' ? 'CRITICAL' : input.category === 'FRAUD' ? 'HIGH' : 'NORMAL',
        dueAt: new Date(Date.now() + dueDays * 86_400_000),
        attachments: attachmentFileIds.length
          ? { create: attachmentFileIds.map((fileAssetId) => ({ fileAssetId, uploadedBy: user?.userId || null })) }
          : undefined,
      },
    })
    ...
    if (recipient)
      await enqueueEmail({
        recipient,
        subject: `FRAD case received: ${referenceNumber}`,
        html: `<p>We received your submission. Your reference is <strong>${referenceNumber}</strong>. Keep this reference for follow-up.</p>`,
        deduplicationKey: `complaint-ack:${created.id}`,
      })
    await prisma.complaintCase.update({
      where: { id: created.id },
      data: { acknowledgementSentAt: recipient ? new Date() : null },
    })
    await logAudit({
      actorUserId: user?.userId,
      action: 'COMPLAINT_CASE_CREATED',
      resourceType: 'ComplaintCase',
      resourceId: created.id,
      newValue: { category: created.category, referenceNumber },
    })
