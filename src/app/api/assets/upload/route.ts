...
export async function POST(request: Request) {
  try {
    ...
    if (!['STANDARD', 'CONFIDENTIAL', 'RESTRICTED'].includes(sensitivityClass))
      return NextResponse.json({ error: 'Invalid sensitivity classification' }, { status: 400 })
    const since = new Date(Date.now() - 24 * 3_600_000)
    const usage = await prisma.fileAsset.aggregate({
      where: { ownerUserId: user.userId, createdAt: { gte: since } },
      _sum: { sizeBytes: true },
    })
    if ((usage._sum.sizeBytes || 0) + file.size > 100 * 1024 * 1024)
      return NextResponse.json({ error: 'Daily upload quota exceeded' }, { status: 429 })
