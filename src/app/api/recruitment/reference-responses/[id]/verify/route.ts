...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const response = await prisma.referenceResponse.findUnique({ where: { id: params.id } })
    if (!response) return NextResponse.json({ error: 'Reference response not found' }, { status: 404 })
    const updated = await prisma.referenceResponse.update({
      where: { id: response.id },
      data: { verifiedBy: user.userId, verifiedAt: new Date() },
    })
    await logAudit({
