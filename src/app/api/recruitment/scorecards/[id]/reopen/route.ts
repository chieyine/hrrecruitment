...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    await prisma.candidateScorecard.update({ where: { id: scorecard.id }, data: { status: 'REOPENED', reopenedBy: user.userId, reopenedAt: new Date(), reopenReason: reason } })
    await logAudit({ actorUserId: user.userId, action: 'SCORECARD_REOPENED', resourceType: 'CandidateScorecard', resourceId: scorecard.id, reason })
