...
export async function GET(request: Request) {
  try {
    ...
    const ids = [...new Set((new URL(request.url).searchParams.get('ids') || '').split(',').filter(Boolean))].slice(
      0,
      100
    )
    ...
    const csv = rowsToCsv(
      records.map((record) => ({
        ...
        Phone: record.candidate.primaryPhone || record.candidate.user.phone || '',
    ...
    const run = await prisma.bulkActionRun.create({
      data: {
        ...
        failedCount: 0,
        status: 'COMPLETED',
        requestJson: JSON.stringify({ ids }),
        resultJson: JSON.stringify({ rows: records.length }),
      },
    })
    await logAudit({
      actorUserId: user.userId,
      action: 'BULK_APPLICATION_EXPORT',
      resourceType: 'BulkActionRun',
      resourceId: run.id,
      newValue: { applicationIds: ids },
    })
    return new Response(csv, {
      headers: {
        'Content-Type': 'text/csv; charset=utf-8',
        'Content-Disposition': `attachment; filename="selected-applications-${new Date().toISOString().slice(0, 10)}.csv"`,
      },
    })
  } catch (error) {
