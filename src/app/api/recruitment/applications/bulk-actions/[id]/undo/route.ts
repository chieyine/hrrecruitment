...
export async function POST(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (run.requestedBy !== user.userId && !user.roles.includes('SYSTEM_ADMIN'))
      throw new AuthzError('Only the original operator or a system administrator can undo this action', 403)
