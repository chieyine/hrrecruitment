...
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const handoverSummary = {
      ...
      candidateFullName: `${application.candidate.legalFirstName} ${application.candidate.middleName || ''} ${application.candidate.lastName}`.trim(),
      ...
      verifiedDocuments: application.candidate.documents.filter((document) => document.status === 'APPROVED').map((document) => ({ id: document.fileAssetId, type: document.documentType, name: document.fileAsset.originalName })),
      educationRecords: application.candidate.education.length,
      employmentRecords: application.candidate.employment.length,
      verifiedLicences: application.candidate.licences.filter((licence) => licence.verificationStatus === 'VERIFIED').length,
      preboarding: application.preboardings[0] ? {
        ...
        informationAcknowledged: application.preboardings[0].infoItems.filter((item) => !item.acknowledgementRequired || item.acknowledgedAt).length,
        readinessChecks: application.preboardings[0].readinessChecks.map((item) => ({ check: item.checkType, status: item.status })),
      } : null,
    }
