    ...
    const record = await prisma.$transaction(async (tx) => {
      const saved = await tx.resumptionRecord.upsert({
        ...
        update: {
          ...
          plannedStartDate: data.outcome === 'POSTPONED' && data.actualStartDate ? data.actualStartDate : undefined,
        },
        create: {
          applicationId: application.id,
          plannedStartDate: application.offers[0]?.startDate || data.actualStartDate || new Date(),
          ...data,
          confirmedBy: user.userId,
        },
      })
