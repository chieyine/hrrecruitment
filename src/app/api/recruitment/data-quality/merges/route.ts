...
async function preview(primaryCandidateId: string, duplicateCandidateId: string) {
  ...
  const records = await prisma.candidateProfile.findMany({
    where: { id: { in: [primaryCandidateId, duplicateCandidateId] } },
    include: {
      user: { select: { email: true, phone: true, accountStatus: true } },
      applications: {
        select: { id: true, vacancyId: true, vacancy: { select: { referenceNumber: true, title: true } } },
      },
      talentPoolMemberships: { select: { talentPoolId: true } },
      _count: {
        select: {
          education: true,
          employment: true,
  ...
  const applicationConflicts = duplicate.applications
    .filter((item) => primaryVacancies.has(item.vacancyId))
    .map((item) => ({ vacancy: item.vacancy.referenceNumber, title: item.vacancy.title }))
  ...
  return {
    ...
    fields: profileFields.map((field) => ({
      ...
      duplicate: duplicate[field] ?? null,
    })),
  ...
  try {
    ...
    const input = await parseBody(
      ...
      z.object({
        ...
        duplicateCandidateId: z.string().min(1),
        reason: z.string().trim().min(10).max(1000),
        survivorChoices: z.record(z.enum(['PRIMARY', 'DUPLICATE'])).default({}),
    ...
    if (!result.canMerge)
      throw new AuthzError('Resolve the listed application or talent-pool conflicts before requesting a merge', 409)
    const review = await prisma.candidateMergeReview.upsert({
      where: {
        primaryCandidateId_duplicateCandidateId: {
      ...
      update: {
        ...
        previewJson: JSON.stringify(result),
        survivorChoicesJson: JSON.stringify(input.survivorChoices),
        reason: input.reason,
        approvedBy: null,
        approvedAt: null,
        mergedAt: null,
    ...
    await logAudit({
      ...
      action: 'CANDIDATE_MERGE_REQUESTED',
      resourceType: 'CandidateMergeReview',
      resourceId: review.id,
      reason: input.reason,
      newValue: {
        primaryCandidateId: input.primaryCandidateId,
        duplicateCandidateId: input.duplicateCandidateId,
        survivorChoices: input.survivorChoices,
      },
    })
  ...
  try {
    ...
    const input = await parseBody(
      ...
      z.object({
        ...
        action: z.enum(['SUBMIT', 'APPROVE', 'REJECT', 'MERGE']),
    ...
    if (review.lockVersion !== input.lockVersion)
      throw new AuthzError('This review changed while you were viewing it. Refresh and try again.', 409)
    if (input.action === 'SUBMIT') {
      if (review.status !== 'DRAFT' || review.requestedBy !== user.userId)
        throw new AuthzError('Only the review owner can submit this draft', 409)
      ...
      for (const field of profileFields)
        if (choices[field] === 'DUPLICATE') survivorData[field] = latest.duplicate[field]
      await prisma.$transaction(async (tx) => {
        ...
        await tx.candidateEducation.updateMany({
          where: { candidateId: review.duplicateCandidateId },
          data: { candidateId: review.primaryCandidateId },
        })
        await tx.candidateEmployment.updateMany({
          where: { candidateId: review.duplicateCandidateId },
          data: { candidateId: review.primaryCandidateId },
        })
        await tx.candidateLicence.updateMany({
          where: { candidateId: review.duplicateCandidateId },
          data: { candidateId: review.primaryCandidateId },
        })
        ...
        await tx.talentPoolMember.updateMany({
          where: { candidateId: review.duplicateCandidateId },
          data: { candidateId: review.primaryCandidateId },
        })
        ...
        await tx.fileAsset.updateMany({
          where: { ownerUserId: latest.duplicate.userId },
          data: { ownerUserId: latest.primary.userId },
        })
        await tx.candidateProfile.delete({ where: { id: review.duplicateCandidateId } })
        await tx.user.update({
          where: { id: latest.duplicate.userId },
          data: { accountStatus: 'MERGED', sessionVersion: { increment: 1 } },
        })
        await tx.candidateMergeReview.update({
          where: { id: review.id },
          data: { status: 'MERGED', mergedAt: new Date(), lockVersion: { increment: 1 } },
        })
      })
    }
    await logAudit({
      actorUserId: user.userId,
      action: `CANDIDATE_MERGE_${input.action}`,
      resourceType: 'CandidateMergeReview',
      resourceId: review.id,
      reason: input.reason,
      newValue: { primaryCandidateId: review.primaryCandidateId, duplicateCandidateId: review.duplicateCandidateId },
    })
