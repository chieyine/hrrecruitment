...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const interview = await prisma.interview.findUnique({ where: { id: params.id }, include: { application: { include: { candidate: true } } } })
    ...
    await prisma.application.update({ where: { id: interview.applicationId }, data: { internalStatus: 'INTERVIEW_INVITED', candidateVisibleStatus: 'INTERVIEW_INVITED', lockVersion: { increment: 1 } } })
    if (interview.application.candidate.userId) {
      await createNotification({
        userId: interview.application.candidate.userId,
        type: 'INTERVIEW_INVITED', title: 'Interview invitation',
        body: message?.trim() || `You have been invited to ${interview.title} on ${interview.scheduledStart.toLocaleString('en-NG', { timeZone: interview.timezone })}.`,
      })
    }
    await logAudit({ actorUserId: user.userId, action: 'INTERVIEW_INVITATION_SENT', resourceType: 'Interview', resourceId: interview.id })
