  ...
  .superRefine((input, context) => {
    if (input.comment !== undefined && input.comment.length < 5) {
      context.addIssue({
        code: 'custom',
        path: ['comment'],
        message: 'Give a short confirmation note or leave it blank',
      })
    }
  })
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const access = canManage
      ...
      : { readAll: false, vacancyOwner: false }
    const chair = interview.panelMembers.find((member) => member.panelRole === 'CHAIR')
    if (chair?.userId !== user.userId && !access.readAll && !access.vacancyOwner)
      throw new AuthzError('Only the panel chair or responsible recruitment team may confirm this outcome', 403)
    if (interview.panelMembers.length === 0 || interview.panelSubmissions.length !== interview.panelMembers.length)
      throw new AuthzError('Every panel member must submit an independent score before confirmation', 409)
    if (interview.panelConfirmedAt) throw new AuthzError('This panel outcome has already been confirmed', 409)
    if (['WITHDRAWN', 'CANCELLED'].includes(interview.application.internalStatus))
      throw new AuthzError('This application is closed', 409)
    if (interview.varianceFlag && !input.comment)
      throw new AuthzError('Explain how the panel resolved the score variance before confirming', 422)

    const average =
      interview.panelSubmissions.reduce((sum, item) => sum + item.totalScore, 0) / interview.panelSubmissions.length
    await prisma.$transaction([
      ...
      prisma.application.update({
        ...
        data:
          interview.application.internalStatus === 'INTERVIEW_INVITED'
            ? {
                interviewScore: average,
                internalStatus: 'INTERVIEW_COMPLETED',
