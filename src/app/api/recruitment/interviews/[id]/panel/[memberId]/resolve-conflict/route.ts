...
export async function POST(request: Request, context: { params: Promise<{ id: string; memberId: string }> }) {
  ...
  try {
    ...
    if (!member) throw new AuthzError('Panel member not found', 404)
    if (member.userId === user.userId) throw new AuthzError('You cannot approve your own conflict exception', 409)
    if (member.conflictStatus === 'NONE') throw new AuthzError('No conflict has been declared', 409)
    const original = { status: member.conflictStatus, comment: member.conflictComment }
    await prisma.interviewPanelMember.update({
      where: { id: member.id },
      data: {
        conflictStatus: 'RESOLVED_EXCEPTION',
        conflictComment: `${member.conflictComment || ''}\nResolution: ${resolution}`,
      },
    })
    await logAudit({
      actorUserId: user.userId,
      action: 'INTERVIEW_CONFLICT_EXCEPTION_APPROVED',
      resourceType: 'InterviewPanelMember',
