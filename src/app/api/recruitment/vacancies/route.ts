...
export async function GET() {
  try {
    ...
    const [vacancies, departments, dutyStations, projects, categories, scorecards, packages] = await Promise.all([
      prisma.vacancy.findMany({ where: vacancyWhere, include: { department: true, dutyStation: true, category: true, _count: { select: { applications: true } } }, orderBy: { createdAt: 'desc' } }),
    ...
    return NextResponse.json({ vacancies, departments, dutyStations, projects, categories, scorecards, packages })
