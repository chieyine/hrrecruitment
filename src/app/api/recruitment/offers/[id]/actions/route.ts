  ...
  try {
    ...
    if (input.action === 'APPROVE') {
      ...
      const approval = await prisma.approval.findFirst({
        where: { resourceType: 'OFFER', resourceId: offer.id, decision: 'PENDING' },
        orderBy: { id: 'desc' },
      })
      if (!approval) return NextResponse.json({ error: 'No pending approval exists' }, { status: 409 })
      if (approval.requestedBy === user.userId)
        return NextResponse.json({ error: 'The offer creator cannot approve their own offer' }, { status: 409 })
      ...
      await prisma.$transaction(async (tx) => {
        ...
        const updated = await tx.offer.updateMany({
          where: { id: offer.id, status: 'PENDING_APPROVAL', version: offer.version },
      ...
      if (!offerFileId) {
        ...
        const variables: Record<string, string> = {
          ...
          duty_station: offer.dutyStation,
          contract_type: offer.contractType,
          contract_duration: offer.contractDuration || '',
          salary: offer.salary,
          start_date: offer.startDate.toLocaleDateString('en-GB'),
          end_date: offer.endDate?.toLocaleDateString('en-GB') || '',
          probation_period: offer.probationPeriod || '',
          reporting_line: offer.reportingLine || '',
          conditions: offer.conditions || '',
          acceptance_deadline: offer.acceptanceDeadline.toLocaleDateString('en-GB'),
        }
        const template =
          offer.offerTemplate?.bodyTemplate ||
