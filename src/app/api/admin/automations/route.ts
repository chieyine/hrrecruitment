...
export async function GET() {
  try {
    ...
    return Response.json({
      controls: AUTOMATIONS.map((definition) => ({
        ...definition,
        mode: byCode.get(definition.code)?.mode || 'ACTIVE',
        settingsJson: byCode.get(definition.code)?.settingsJson || '{}',
        updatedAt: byCode.get(definition.code)?.updatedAt || null,
      })),
      recent,
    })
  } catch (error) {
...
export async function PATCH(request: Request) {
  try {
    ...
    if (body.settingsJson) {
      try {
        JSON.parse(body.settingsJson)
      } catch {
        throw new AuthzError('Settings must be valid JSON', 400)
      }
    }
    ...
    const control = await prisma.automationControl.upsert({
      ...
      update: {
        mode: body.mode,
        settingsJson: body.settingsJson || previous?.settingsJson || '{}',
        updatedBy: user.userId,
        ...(body.mode === 'PREVIEW' ? { lastPreviewAt: new Date() } : {}),
      },
      create: {
        code: body.code,
    ...
    await logAudit({
      ...
      resourceType: 'AutomationControl',
...
export async function POST(request: Request) {
  try {
    ...
    await prisma.automationControl.updateMany({
      where: { code: body.code },
      data: { lastOverriddenAt: new Date(), updatedBy: user.userId },
    })
    await logAudit({
      actorUserId: user.userId,
      action: 'AUTOMATION_OVERRIDDEN',
      resourceType: body.targetType,
      resourceId: body.targetId,
      reason: body.reason,
      newValue: { automationCode: body.code },
    })
