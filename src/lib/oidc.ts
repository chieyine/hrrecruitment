...
export async function oidcConfiguration(issuer: string): Promise<OidcConfiguration> {
  ...
  if (configuration.issuer.replace(/\/$/, '') !== issuer.replace(/\/$/, ''))
    throw new Error('Identity-provider issuer mismatch')
