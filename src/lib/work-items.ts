...
export async function syncOperationalWorkItems() {
  ...
  const [
    ...
    prisma.candidateAssessment.findMany({
      where: { OR: [{ status: { in: ['SUBMITTED', 'AUTO_SUBMITTED'] }, markerUserId: null }, { status: 'AWAITING_APPROVAL', resultApprovedAt: null }] },
      include: {
  ...
  const writes = [
    ...
    ...assessments.map((item) =>
      prisma.workItem.upsert({
        where: { deduplicationKey: `${item.status === 'AWAITING_APPROVAL' ? 'assessment-approval' : 'assessment-marking'}:${item.id}` },
        update: { dueAt: deadline(item.submittedAt || item.invitedAt, assessmentMinutes) },
        create: {
          deduplicationKey: `${item.status === 'AWAITING_APPROVAL' ? 'assessment-approval' : 'assessment-marking'}:${item.id}`,
          workType: item.status === 'AWAITING_APPROVAL' ? 'ASSESSMENT_APPROVAL' : 'ASSESSMENT_MARKING',
          title: `${item.status === 'AWAITING_APPROVAL' ? 'Approve' : 'Mark'} assessment for ${item.application.candidate.legalFirstName} ${item.application.candidate.lastName}`,
          description: `${item.application.vacancy.referenceNumber} · ${item.assessment.title}`,
          priority: 'HIGH',
          assignedRole: item.status === 'AWAITING_APPROVAL' ? 'HR_MANAGER' : 'RECRUITMENT_OFFICER',
  ...
  const activeKeys = new Set([
    ...
    ...assessments.map((item) => `${item.status === 'AWAITING_APPROVAL' ? 'assessment-approval' : 'assessment-marking'}:${item.id}`),
