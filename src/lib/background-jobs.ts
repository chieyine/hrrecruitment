...
import { sendJobAlerts } from './job-alerts'
...
export async function processBackgroundSchedules() {
  ...
  const renewed = await prisma.jobLease.updateMany({
    where: { jobName: 'PROCESS_SCHEDULES', lockedUntil: { lte: now } },
    data: { leaseOwner, lockedUntil: leaseUntil },
  })
  if (!renewed.count) {
    try {
      await prisma.jobLease.create({ data: { jobName: 'PROCESS_SCHEDULES', leaseOwner, lockedUntil: leaseUntil } })
    } catch {
      throw new Error('PROCESS_SCHEDULES is already running')
    }
  }
  ...
  const notifyOnceToday = async (userId: string, type: string, title: string, body: string) => {
    ...
    const exists = await prisma.notification.findFirst({
      where: { userId, type, body, sentAt: { gte: start } },
      select: { id: true },
    })
  ...
  try {
    const vacanciesToOpen = await prisma.vacancy.findMany({
      where: { status: 'SCHEDULED', openingAt: { lte: now }, closingAt: { gt: now } },
      select: { id: true },
    })
    const openedVacanciesResult = await automatedBatch(
      'VACANCY_SCHEDULES',
      'OPEN_VACANCY',
      'Vacancy',
      vacanciesToOpen,
      async (vacancy) => {
        await prisma.vacancy.updateMany({ where: { id: vacancy.id, status: 'SCHEDULED' }, data: { status: 'OPEN' } })
      }
    )

    // 1. Expire outdated vacancies
    const vacanciesToClose = await prisma.vacancy.findMany({
      where: { status: 'OPEN', closingAt: { lt: now } },
      select: { id: true },
    })
    const expiredVacanciesResult = await automatedBatch(
      'VACANCY_SCHEDULES',
      'CLOSE_VACANCY',
      'Vacancy',
      vacanciesToClose,
      async (vacancy) => {
        await prisma.vacancy.updateMany({ where: { id: vacancy.id, status: 'OPEN' }, data: { status: 'CLOSED' } })
      }
    )
    ...
    const offerReminders = await prisma.offer.findMany({
      where: { status: { in: ['SENT', 'VIEWED'] }, acceptanceDeadline: { gt: now, lte: reminderWindow } },
      include: { application: { include: { candidate: true } } },
      take: 500,
    })
    await automatedBatch('OFFER_REMINDERS', 'REMIND_OFFER', 'Offer', offerReminders, async (offer) => {
      await notifyOnceToday(
        offer.application.candidate.userId,
        `OFFER_REMINDER:${offer.id}`,
        'Offer response reminder',
        `Your offer for ${offer.position} expires on ${offer.acceptanceDeadline.toLocaleDateString('en-GB')}.`
      )
    })

    const courseReminders = await prisma.candidateCourse.findMany({
      where: { required: true, status: { notIn: ['COMPLETED', 'WAIVED'] }, dueAt: { lte: reminderWindow } },
      include: { course: true, candidatePreboarding: { include: { application: { include: { candidate: true } } } } },
      take: 500,
    })
    await automatedBatch('PREBOARDING_REMINDERS', 'REMIND_COURSE', 'CandidateCourse', courseReminders, async (item) => {
      await notifyOnceToday(
        item.candidatePreboarding.application.candidate.userId,
        `COURSE_REMINDER:${item.id}`,
        'Preboarding course due',
        `Complete ${item.course.title}${item.dueAt ? ` by ${item.dueAt.toLocaleDateString('en-GB')}` : ''}.`
      )
    })
    const taskReminders = await prisma.candidatePreboardingTask.findMany({
      where: { required: true, status: { notIn: ['COMPLETED', 'APPROVED', 'WAIVED'] }, dueAt: { lte: reminderWindow } },
      include: {
        taskTemplate: true,
        candidatePreboarding: { include: { application: { include: { candidate: true } } } },
      },
      take: 500,
    })
    await automatedBatch(
      'PREBOARDING_REMINDERS',
      'REMIND_TASK',
      ...
      async (item) => {
        await notifyOnceToday(
          item.candidatePreboarding.application.candidate.userId,
          `TASK_REMINDER:${item.id}`,
          'Preboarding task due',
          `Complete ${item.taskTemplate.title}${item.dueAt ? ` by ${item.dueAt.toLocaleDateString('en-GB')}` : ''}.`
        )
      }
    )
    const formReminders = await prisma.candidatePreboardingForm.findMany({
      where: { required: true, status: { notIn: ['APPROVED', 'WAIVED'] }, dueAt: { lte: reminderWindow } },
      include: {
        formTemplate: true,
        candidatePreboarding: { include: { application: { include: { candidate: true } } } },
      },
    ...
    await automatedBatch(
      ...
      async (item) => {
        await notifyOnceToday(
          item.candidatePreboarding.application.candidate.userId,
          `FORM_REMINDER:${item.id}`,
          'Preboarding form due',
          `Complete ${item.formTemplate.title}${item.dueAt ? ` by ${item.dueAt.toLocaleDateString('en-GB')}` : ''}.`
        )
      }
    )
    const documentReminders = await prisma.candidateRequiredDocument.findMany({
      where: { required: true, status: { notIn: ['APPROVED', 'WAIVED'] }, dueAt: { lte: reminderWindow } },
      include: {
    ...
    await automatedBatch(
      ...
      async (item) => {
        await notifyOnceToday(
          item.candidatePreboarding.application.candidate.userId,
          `DOCUMENT_REMINDER:${item.id}`,
          'Preboarding document due',
          `Submit ${item.documentRequirement.name}${item.dueAt ? ` by ${item.dueAt.toLocaleDateString('en-GB')}` : ''}.`
        )
      }
    )
    const policyReminders = await prisma.candidatePolicyAcknowledgement.findMany({
      where: { required: true, status: { notIn: ['SIGNED', 'APPROVED', 'WAIVED'] }, dueAt: { lte: reminderWindow } },
      include: {
        policyDocument: true,
        candidatePreboarding: { include: { application: { include: { candidate: true } } } },
      },
    ...
    await automatedBatch(
      ...
      async (item) => {
        await notifyOnceToday(
          item.candidatePreboarding.application.candidate.userId,
          `POLICY_REMINDER:${item.id}`,
          'Policy signature due',
          `Read and sign ${item.policyDocument.title}${item.dueAt ? ` by ${item.dueAt.toLocaleDateString('en-GB')}` : ''}.`
        )
      }
    )
    ...
    const draftApplications = await prisma.application.findMany({
      where: {
        ...
        updatedAt: { lte: draftThreshold },
        vacancy: { status: 'OPEN', closingAt: { gt: now } },
      },
      include: { candidate: true, vacancy: true },
      take: 500,
    })
    await automatedBatch('DRAFT_REMINDERS', 'REMIND_DRAFT', 'Application', draftApplications, async (application) => {
      await notifyOnceToday(
        application.candidate.userId,
        `APPLICATION_DRAFT_REMINDER:${application.id}`,
        'Complete your application',
        `Your draft application for ${application.vacancy.title} has not been submitted. It closes on ${application.vacancy.closingAt.toLocaleDateString('en-GB')}.`
      )
    })

    const assessmentReminders = await prisma.candidateAssessment.findMany({
      where: {
        status: { in: ['INVITED', 'NOT_STARTED'] },
        assessment: { OR: [{ opensAt: { lte: reminderWindow } }, { closesAt: { lte: reminderWindow } }] },
      },
      include: { assessment: true, application: { include: { candidate: true } } },
      take: 500,
    })
    await automatedBatch(
      ...
      async (record) => {
        await notifyOnceToday(
          record.application.candidate.userId,
          `ASSESSMENT_REMINDER:${record.id}`,
          'Assessment reminder',
          `${record.assessment.title} is ready or approaching its deadline${record.assessment.closesAt ? ` on ${record.assessment.closesAt.toLocaleDateString('en-GB')}` : ''}.`
        )
      }
    )
    ...
    const assessmentInvitationCandidates = await prisma.application.findMany({
      ...
      include: { candidate: true, vacancy: { include: { assessments: true } } },
      take: 500,
    })
    const unambiguousAssessmentInvitations = assessmentInvitationCandidates.filter(
      (application) =>
        application.vacancy.assessments.length === 1 &&
        (!application.vacancy.assessments[0].closesAt || application.vacancy.assessments[0].closesAt > now)
    )
    await automatedBatch(
      'ASSESSMENT_INVITATIONS',
      'INVITE_ASSESSMENT',
      'Application',
      unambiguousAssessmentInvitations,
      async (application) => {
        ...
        await createNotification({
          ...
          body: `An assessment for ${application.vacancy.title} is ready in your account.`,
        })
      }
    )

    const interviewWindow = new Date(now.getTime() + 86400000)
    const interviewReminders = await prisma.interview.findMany({
      where: {
        status: { in: ['SCHEDULED', 'CONFIRMED', 'RESCHEDULED'] },
        scheduledStart: { gt: now, lte: interviewWindow },
      },
      include: { application: { include: { candidate: true } }, panelMembers: true },
      take: 500,
    })
    await automatedBatch(
      'INTERVIEW_REMINDERS',
      ...
      async (interview) => {
        await notifyOnceToday(
          ...
          `INTERVIEW_REMINDER:${interview.id}`,
          'Interview reminder',
          `${interview.title} starts on ${interview.scheduledStart.toLocaleString('en-GB')}.`
        )
        for (const member of interview.panelMembers)
          await notifyOnceToday(
            member.userId,
            `PANEL_INTERVIEW_REMINDER:${interview.id}`,
            'Panel interview reminder',
            `${interview.title} starts on ${interview.scheduledStart.toLocaleString('en-GB')}.`
          )
      }
    )

    const referenceReminders = await prisma.referenceRequest.findMany({
      where: {
        status: { in: ['PENDING', 'SENT'] },
        expiresAt: { gt: now, lte: reminderWindow },
        OR: [{ reminderSentAt: null }, { reminderSentAt: { lt: new Date(now.getTime() - 86400000) } }],
      },
    ...
    await automatedBatch(
      ...
      async (request) => {
        await enqueueEmail({
          recipient: request.referee.email,
          subject: 'Reminder: FRAD reference request',
          html: '<p>Please complete the confidential FRAD reference request using the original secure link before it expires.</p>',
          deduplicationKey: `automatic-reference-reminder:${request.id}:${now.toISOString().slice(0, 10)}`,
        })
    ...
    const notReadySoon = await prisma.candidatePreboarding.findMany({
      where: { confirmedStartDate: { gt: now, lte: resumptionWindow }, readinessStatus: { not: 'READY_TO_RESUME' } },
      include: { application: { include: { candidate: true, vacancy: true } } },
      take: 500,
    })
    if (notReadySoon.length) {
      const managers = await prisma.user.findMany({
        where: { accountStatus: 'ACTIVE', userRoles: { some: { role: { name: 'HR_MANAGER' } } } },
        select: { id: true },
      })
      await automatedBatch(
        'READINESS_ALERTS',
        ...
        async (record) => {
          for (const manager of managers)
            await notifyOnceToday(
              manager.id,
              `NOT_READY_TO_RESUME:${record.id}`,
              'Candidate not ready to resume',
              `${record.application.candidate.legalFirstName} ${record.application.candidate.lastName} is due to resume ${record.confirmedStartDate!.toLocaleDateString('en-GB')} for ${record.application.vacancy.title}, but readiness is ${record.readinessStatus}.`
            )
        }
      )
    }
    ...
    await automatedBatch('SCHEDULED_REPORTS', 'SEND_REPORT', 'ScheduledReport', dueReports, async (schedule) => {
      ...
      await enqueueEmail({
        ...
        html: '<p>Your scheduled FRAD report is attached. It contains controlled information; store and share it under FRAD privacy and retention rules.</p>',
        attachments: [attachment],
        deduplicationKey: `scheduled-report:${schedule.id}:${schedule.nextRunAt.toISOString()}`,
      })
    ...
    const dueConfigurationReleases = await prisma.configurationChangeRequest.findMany({
      where: { status: 'APPROVED', changeType: { startsWith: 'GENERIC_CONFIG_UPDATE:' }, scheduledFor: { lte: now } },
      take: 100,
    })
    for (const release of dueConfigurationReleases) {
      try {
        ...
        await prisma.configurationChangeRequest.update({
          ...
          data: {
            status: 'FAILED',
            decisionComment: error instanceof Error ? error.message : 'Scheduled publication failed',
            lockVersion: { increment: 1 },
          },
        })
      }
    }
    const expiredConfigurationReleases = await prisma.configurationChangeRequest.findMany({
      where: { status: 'APPLIED', effectiveTo: { lte: now } },
      take: 100,
    })
    for (const release of expiredConfigurationReleases) await expireConfigurationRelease(release.id)

    const soon = new Date(now.getTime() + 30 * 86400000)
    const expiringLicences = await prisma.candidateLicence.findMany({
      where: { expiryDate: { gt: now, lte: soon } },
      include: { candidate: true },
      take: 500,
    })
    for (const licence of expiringLicences)
      await notifyOnceToday(
        licence.candidate.userId,
        `LICENCE_EXPIRING:${licence.id}`,
        'Professional licence expiring',
        `${licence.licenceType} expires on ${licence.expiryDate!.toLocaleDateString('en-GB')}. Upload renewed evidence when available.`
      )
    ...
    await automatedBatch(
      'ASSESSMENT_AUTO_SUBMIT',
      'AUTO_SUBMIT',
      'CandidateAssessment',
      expiredCandidateAssessments,
      async (ca) => {
        const answerMap = new Map(
          ca.answers.map((answer) => {
            try {
              return [answer.assessmentQuestionId, answer.answerJson ? JSON.parse(answer.answerJson) : null] as const
            } catch {
              return [answer.assessmentQuestionId, answer.answerJson] as const
            }
          })
        )
        ...
        for (const question of ca.assessment.questions) {
          ...
          const expected =
            Array.isArray(correct) && correct.length === 1 && !Array.isArray(actual) ? correct[0] : correct
    ...
    const decisionItems = await prisma.workItem.findMany({
      where: {
        status: { in: ['OPEN', 'IN_PROGRESS', 'BLOCKED'] },
        assignedUserId: { not: null },
        assignedUser: { userRoles: { some: { role: { name: { in: ['HIRING_MANAGER', 'APPROVER'] } } } } },
        OR: [{ dueAt: { lte: reminderWindow } }, { priority: 'URGENT' }],
      },
      take: 500,
    })
    await automatedBatch('DECISION_REMINDERS', 'REMIND_DECISION_OWNER', 'WorkItem', decisionItems, async (item) => {
      await notifyOnceToday(
        item.assignedUserId!,
        `DECISION_REMINDER:${item.id}`,
        'Recruitment decision required',
        `${item.title}${item.dueAt ? ` is due ${item.dueAt.toLocaleDateString('en-GB')}` : ' needs your attention'}.`
      )
    })
    const escalationCandidates = await prisma.workItem.findMany({
      where: { status: { in: ['OPEN', 'IN_PROGRESS', 'BLOCKED'] }, dueAt: { lt: now }, escalatedAt: null },
      select: { id: true },
    })
    const escalatedWork = await automatedBatch(
      'WORK_ESCALATION',
      'ESCALATE_WORK',
      'WorkItem',
      ...
      async (item) => {
        await prisma.workItem.updateMany({
          ...
          data: { escalatedAt: now, escalationLevel: { increment: 1 }, priority: 'URGENT' },
        })
      }
    )
    ...
    const jobAlerts = await sendJobAlerts(now)

    const outbox = await processOutboxBatch()
    const retention = await runRetentionPolicy(now)
    // Batch size is configurable: a bulk upload can otherwise outrun a fixed 25
    // per tick and leave files undownloadable for longer than expected.
    const rescanBatchSize = Math.max(1, Math.min(500, Number(process.env.VIRUS_RESCAN_BATCH_SIZE || 25)))
    const pendingFiles = await prisma.fileAsset.findMany({
      where: { virusScanStatus: 'PENDING' },
      take: rescanBatchSize,
    })
    let cleanFiles = 0,
      infectedFiles = 0
    for (const file of pendingFiles) {
      ...
      if (status === 'CLEAN') {
        await prisma.fileAsset.update({ where: { id: file.id }, data: { virusScanStatus: 'CLEAN' } })
        cleanFiles++
      } else if (status === 'INFECTED') {
        await deleteStoredFile(file.storageKey)
        await prisma.fileAsset.update({ where: { id: file.id }, data: { virusScanStatus: 'INFECTED' } })
        await prisma.operationalEvent.create({
          data: { eventType: 'MALWARE_DETECTED', severity: 'CRITICAL', resourceType: 'FileAsset', resourceId: file.id },
        })
        infectedFiles++
      }
    }
    const summary = {
      ...
      cleanFiles,
      infectedFiles,
      stillPendingFiles: await prisma.fileAsset.count({ where: { virusScanStatus: 'PENDING' } }),
      jobAlerts,
    }
