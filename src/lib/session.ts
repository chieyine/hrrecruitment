...
/**
 * Production normally means HTTPS, but production-mode local acceptance tests
 * deliberately run at an http:// loopback APP_URL. A Secure cookie is ignored
 * by browsers on that origin, leaving a successful login stranded on the
 * login page. Derive the attribute from the configured public origin when it
 * is present; fall back to the environment mode for older deployments.
 */
export function shouldUseSecureCookies(): boolean {
  ...
  if (appUrl) {
    try {
      ...
      // Configuration validation elsewhere will surface the malformed URL.
    }
  }
