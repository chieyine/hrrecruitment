// Shared server-side report implementation used by both HTTP exports and scheduled delivery.
...
const REPORT_TYPES = new Set<string>(REPORT_TYPE_VALUES)
...
async function endToEndReportRows(
  ...
  if (type === 'source-of-application') {
    ...
    for (const record of records) {
      const key = `${record.vacancy.referenceNumber}:${record.source}`
      const entry =
        grouped.get(key) ?? { total: 0, hired: 0, shortlisted: 0, audience: record.source, title: record.vacancy.title }
      entry.total += 1
