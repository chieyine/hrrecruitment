...
export async function instantiatePreboardingPackage(
  ...
  await db.candidatePreboardingPackage.upsert({
    where: { candidatePreboardingId_preboardingPackageId: { candidatePreboardingId, preboardingPackageId: pkg.id } },
    update: {},
    create: { candidatePreboardingId, preboardingPackageId: pkg.id, assignedBy },
  })
  ...
  const [forms, documents, policies, courses, tasks] = await Promise.all([
    ...
    db.candidateRequiredDocument.findMany({
      where: { candidatePreboardingId },
      select: { documentRequirementId: true },
    }),
    db.candidatePolicyAcknowledgement.findMany({
      where: { candidatePreboardingId },
  ...
  await Promise.all([
    ...pkg.packageForms
      .filter((item) => !formIds.has(item.formTemplateId))
      .map((item) =>
        db.candidatePreboardingForm.upsert({
          where: {
            candidatePreboardingId_formTemplateId: { candidatePreboardingId, formTemplateId: item.formTemplateId },
          },
          update: {},
          create: {
            candidatePreboardingId,
            formTemplateId: item.formTemplateId,
            required: item.required,
            dueAt: dueAt(item.dueOffsetDays),
            templateSnapshotJson: JSON.stringify(item.formTemplate),
          },
        })
      ),
    ...pkg.packageDocuments
      .filter((item) => !documentIds.has(item.documentRequirementId))
      .map((item) =>
        db.candidateRequiredDocument.upsert({
          where: {
            candidatePreboardingId_documentRequirementId: {
              candidatePreboardingId,
              documentRequirementId: item.documentRequirementId,
            },
          },
          update: {},
          create: {
            ...
            required: item.required,
            dueAt: dueAt(item.dueOffsetDays),
            requirementSnapshotJson: JSON.stringify(item.documentRequirement),
          },
        })
      ),
    ...pkg.packagePolicies
      .filter((item) => !policyIds.has(item.policyDocumentId))
      .map((item) =>
        db.candidatePolicyAcknowledgement.upsert({
          where: {
            candidatePreboardingId_policyDocumentId: {
              candidatePreboardingId,
          ...
          create: {
            ...
            required: item.required,
            dueAt: dueAt(item.dueOffsetDays),
            policySnapshotJson: JSON.stringify(item.policyDocument),
          },
        })
      ),
    ...pkg.packageCourses
      ...
      .map((item) =>
        db.candidateCourse.upsert({
          where: { candidatePreboardingId_courseId: { candidatePreboardingId, courseId: item.courseId } },
          update: {},
          create: {
            candidatePreboardingId,
            ...
            dueAt: dueAt(item.dueOffsetDays),
            required: item.required,
            courseSnapshotJson: JSON.stringify({ ...item.course, timing: item.timing }),
          },
        })
      ),
    ...pkg.packageTasks
      .filter((item) => !taskIds.has(item.taskTemplateId))
      .map((item) =>
        db.candidatePreboardingTask.upsert({
          where: {
            candidatePreboardingId_taskTemplateId: { candidatePreboardingId, taskTemplateId: item.taskTemplateId },
          },
          update: {},
          create: {
            candidatePreboardingId,
            ...
            dueAt: dueAt(item.dueOffsetDays),
            required: item.required,
            taskSnapshotJson: JSON.stringify(item.taskTemplate),
          },
        })
      ),
  ])
...
export async function refreshPreboardingProgress(candidatePreboardingId: string) {
  ...
  const all = groups.flatMap(([name, items]) =>
    items.map((rawItem) => {
      ...
      return {
        ...
        status:
          name === 'information'
            ? 'acknowledgedAt' in item && item.acknowledgedAt
              ? 'ACKNOWLEDGED'
              : 'PENDING'
            : item.status,
        required:
          (name === 'information'
            ? 'acknowledgementRequired' in item && item.acknowledgementRequired
            : item.required) &&
  ...
  const mandatoryComplete =
    mandatory.length > 0 &&
    mandatory.every((item) => COMPLETE_STATUSES[item.name].has(item.status)) &&
    startDateComplete
...
export async function syncReadinessChecks(candidatePreboardingId: string) {
  ...
  const idDocs = pb.documents.filter(
    (item) => item.required && /IDENTITY|PASSPORT|NIN/i.test(item.documentRequirement.documentType)
  )
  const qualificationDocs = pb.documents.filter(
    (item) => item.required && /ACADEMIC|QUALIFICATION|CERTIFICATE/i.test(item.documentRequirement.documentType)
  )
  const states: Record<string, boolean> = {
    ...
    FORMS_APPROVED: allRequired(
      pb.forms.filter((item) => item.required),
      ['APPROVED', 'WAIVED']
    ),
    POLICIES_SIGNED: allRequired(
      pb.policyAcknowledgements.filter((item) => item.required),
      ['SIGNED', 'APPROVED', 'WAIVED']
    ),
    COURSES_COMPLETED: allRequired(
      pb.courses.filter((item) => item.required && courseBlocksReadiness(item.courseSnapshotJson)),
      ['COMPLETED', 'WAIVED']
    ),
    TASKS_COMPLETED: allRequired(
      pb.tasks.filter((item) => item.required),
      ['COMPLETED', 'APPROVED', 'WAIVED']
    ),
    START_DATE_CONFIRMED: Boolean(pb.startDateConfirmedAt),
    REQUIRED_MEETINGS: allRequired(
      pb.meetings.filter((item) => item.required),
      ['ATTENDED', 'WAIVED']
    ),
    ...
    PROFESSIONAL_LICENCE: allRequired(
      pb.documents.filter(
        (item) => item.required && /LICEN[CS]E|PROFESSIONAL REGISTRATION/i.test(item.documentRequirement.documentType)
      ),
      ['APPROVED', 'WAIVED'],
      pb.documents.some(
        (item) => item.required && /LICEN[CS]E|PROFESSIONAL REGISTRATION/i.test(item.documentRequirement.documentType)
    ...
    MEDICAL_CLEARANCE: allRequired(
      pb.documents.filter(
        (item) => item.required && /MEDICAL|FITNESS|HEALTH CLEARANCE/i.test(item.documentRequirement.documentType)
      ),
      ['APPROVED', 'WAIVED'],
      pb.documents.some(
        (item) => item.required && /MEDICAL|FITNESS|HEALTH CLEARANCE/i.test(item.documentRequirement.documentType)
  ...
  await Promise.all(
    Object.entries(states).map(([checkType, passed]) =>
