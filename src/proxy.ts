...
export async function proxy(req: NextRequest) {
  ...
  if (isStaffArea(pathname)) {
    const isStaff = isAdmin || roles.some((r) => r !== 'CANDIDATE')
    ...
    if (isAdminArea(pathname) && !isAdmin) {
      ...
      const hrManagerPath = pathname === '/admin/automations' || pathname === '/api/admin/automations'
      if (!(isCourseAdmin && courseAdminPath) && !(isHrManager && hrManagerPath)) return deny(403, '/recruitment/dashboard')
    }
  }
