...
Requirements: Node.js 20+, npm, and SQLite for local development.
...
The integration suite creates an isolated temporary SQLite database. The
production build runs Prisma Client generation before compiling Next.js.
...
- Admin includes users/roles/status, departments, projects, duty stations, contract/document types, vacancy categories, templates, scorecards, courses, policies, packages, settings, and privacy requests.
...
Local development uses SQLite and AES-256-GCM encrypted filesystem-backed private storage. Production
